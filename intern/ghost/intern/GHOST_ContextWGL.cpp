...
// === Globais para manter estado do contexto moderno ===
static bool contextInitialized = false;
static HGLRC modernContext = NULL;
static HDC dummyHDC = NULL;
static HWND dummyHWND = NULL;
static GLuint pingTexture = 0;
static GLuint pongTexture = 0;
static bool firstCall = true;

// === Globais para shader compilado e reuso ===
static GLuint cachedProgram = 0;
static std::string cachedShaderSrc;

GHOST_TSuccess runComputeShader(
    const char* shaderSrc,
    GLuint textureID1,
    GLuint textureID2,
    GLuint textureID3,
    bool useFluidSim,
    const std::vector<ComputeUniform>& uniformsRaw
)
{
    static bool contextChecked = false;
    static bool contextSuficiente = false;

    static GLuint ubo = 0;
    static std::vector<ComputeUniform> lastUniforms;

    std::vector<ComputeUniform> uniforms;
    for (const auto& u : uniformsRaw) {
        auto it = std::find_if(uniforms.begin(), uniforms.end(),
            [&u](const ComputeUniform& existing) {
                return existing.name == u.name;
            });
        if (it == uniforms.end()) {
            uniforms.push_back(u);
        } else if (it->value != u.value) {
            it->value = u.value;
        }
    }

    HGLRC legacyContext = wglGetCurrentContext();
    HDC legacyDC = wglGetCurrentDC();

    // === Verifica o contexto atual apenas uma vez ===
    if (!contextChecked) {
        ...
        contextSuficiente = (major > 4 || (major == 4 && minor >= 3));

        if (contextSuficiente) {
            CM_Logf("Contexto atual é moderno (GL %d.%d). Nenhum novo contexto será criado.", major, minor);
        } else {
            CM_Logf("Contexto atual é antigo (GL %d.%d). Será criado um contexto moderno separado.", major, minor);
        }
        contextChecked = true;
    }

    // === Caso o contexto atual seja suficiente ===
    if (contextSuficiente) {
        wglMakeCurrent(legacyDC, legacyContext);
    } 
    else {
        // === Lógica antiga para criar contexto moderno ===
        if (!contextInitialized) {
            dummyHWND = CreateWindowExW(
                0, L"STATIC", L"ComputeContext", WS_POPUP,
                0, 0, 1, 1, NULL, NULL, GetModuleHandle(NULL), NULL);

            if (!dummyHWND) {
                CM_Error("Falha ao criar janela dummy.");
                return GHOST_kFailure;
            }

            dummyHDC = GetDC(dummyHWND);
            int pf = GetPixelFormat(legacyDC);
            PIXELFORMATDESCRIPTOR pfd;
            DescribePixelFormat(legacyDC, pf, sizeof(pfd), &pfd);
            SetPixelFormat(dummyHDC, pf, &pfd);

            HGLRC tempContext = wglCreateContext(dummyHDC);
            if (!tempContext) {
                CM_Error("Falha ao criar contexto temporário.");
            ...
            PFNWGLCREATECONTEXTATTRIBSARBPROC wglCreateContextAttribsARB_local =
                (PFNWGLCREATECONTEXTATTRIBSARBPROC)wglGetProcAddress("wglCreateContextAttribsARB");

            if (!wglCreateContextAttribsARB_local) {
                CM_Error("wglCreateContextAttribsARB não disponível.");
            ...
            modernContext = wglCreateContextAttribsARB_local(dummyHDC, legacyContext, attribs);
            if (!modernContext) {
                CM_Error("Falha ao criar contexto moderno.");
                wglMakeCurrent(NULL, NULL);
                wglDeleteContext(tempContext);
                ReleaseDC(dummyHWND, dummyHDC);
                DestroyWindow(dummyHWND);
            ...
            CM_Log("Contexto OpenGL moderno inicializado com sucesso.");
        }

        if (!wglMakeCurrent(dummyHDC, modernContext)) {
            CM_Error("Falha ao ativar contexto moderno.");
            return GHOST_kFailure;
        }
    }

    // === Compilar e linkar shader uma vez ===
    if (cachedProgram == 0 || cachedShaderSrc != shaderSrc) {
        if (cachedProgram != 0) {
            glDeleteProgram(cachedProgram);
            cachedProgram = 0;
        }
        ...
        if (!success) {
            ...
            CM_Errorf("Erro ao compilar compute shader:\n%s", log);
        ...
        cachedShaderSrc = shaderSrc;
        CM_Log("Shader compute compilado e cacheado com sucesso.");
    }

    glUseProgram(cachedProgram);

    bool layoutChanged = uniforms.size() != lastUniforms.size();
    if (!layoutChanged) {
        for (size_t i = 0; i < uniforms.size(); ++i) {
            if (uniforms[i].name != lastUniforms[i].name) {
                layoutChanged = true;
                break;
            }
        }
    ...
    if (layoutChanged) {
        ...
        glGenBuffers(1, &ubo);
        CM_Logf("UBO recriado com %zu uniforms.", uniforms.size());
    }

    std::vector<float> values(uniforms.size(), 0.0f);
    for (size_t i = 0; i < uniforms.size(); ++i) {
        values[i] = uniforms[i].value;
    }

    glBindBuffer(GL_UNIFORM_BUFFER, ubo);
    glBufferData(GL_UNIFORM_BUFFER, sizeof(float) * values.size(), values.data(), GL_DYNAMIC_DRAW);
    glBindBufferBase(GL_UNIFORM_BUFFER, 0, ubo);
    lastUniforms = uniforms;

    if (!useFluidSim) {
        glBindImageTexture(0, textureID1, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA8);
        glBindImageTexture(1, textureID2, 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA8);
        glBindImageTexture(2, textureID3, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA8);
    } else {
        glBindImageTexture(0, textureID1, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA8);
        glBindImageTexture(1, pingTexture, 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA8);
        glBindImageTexture(2, pongTexture, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA8);
    }
    ...
    if (useFluidSim) {
        std::swap(pingTexture, pongTexture);
    }
