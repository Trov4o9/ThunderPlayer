...
void rgb_to_hsv(vec4 rgb, out vec4 outcol)
{
    ...
    if (delta != 0.0) {
        vec3 deltaC = (cmax - c) / delta;
        bool isR = c.r == cmax;
        bool isG = c.g == cmax;
        bool isB = c.b == cmax;

        h = isR ? deltaC.b - deltaC.g :
            (isG ? 2.0 + deltaC.r - deltaC.b :
                   4.0 + deltaC.g - deltaC.r);
        h /= 6.0;
        if (h < 0.0) h += 1.0;
    }

    outcol = vec4(h, s, v, rgb.a);
}
...
{
	if (c < 0.04045)
		return (c < 0.0) ? 0.0 : c * 0.07739938080495356;
	else
		return pow(c * 0.9478672985781991 + 0.05213270142180095, 2.4);
}
        ...
        out vec3 normal, out vec4 vcol, out float vcol_alpha, out float frontback)
{
	...
	frontback = float(gl_FrontFacing);
}
...
void point_map_to_sphere(vec3 vin, out vec3 vout)
{
	...
	float v = 0.0, u = 0.0;
	if (len > 0.0) {
		u = (vin.x == 0.0 && vin.y == 0.0) ? 0.0 : (1.0 - atan(vin.x, vin.y) / M_PI) * 0.5;
...
{
	float u = 0.0, v = (vin.z + 1.0) * 0.5;
	float len = length(vin.xy);
	if (len > 0.0) {
		u = (1.0 - atan(vin.x, vin.y) / M_PI) * 0.5;
	}
	else {
		v = 0.0;
	}
...
{
	outval = (abs(val) <= 1.0) ? asin(val) : 0.0;
}

void math_acos(float val, out float outval)
{
	outval = (abs(val) <= 1.0) ? acos(val) : 0.0;
}
...
{
	outvec = v1 + v2;
	outval = dot(abs(outvec), vec3(1.0 / 3.0));
}
...
{
	outvec.x = texture2D(curvemap, vec2(vec.x * 0.5 + 0.5, 0.0)).x;
	outvec.y = texture2D(curvemap, vec2(vec.y * 0.5 + 0.5, 0.0)).y;
	outvec.z = texture2D(curvemap, vec2(vec.z * 0.5 + 0.5, 0.0)).z;
...
void curves_rgb(float fac, vec4 col, sampler2D curvemap, out vec4 outcol)
{
	...
	outcol.rgb = mix(col.rgb, outcol.rgb, fac);
	outcol.a = col.a;
}
...
void mix_screen(float fac, vec4 col1, vec4 col2, out vec4 outcol)
{
	...
	outcol = 1.0 - (facm + fac * (1.0 - col2)) * (1.0 - col1);
	outcol.a = col1.a;
}
...
{
#ifdef USE_NEW_SHADING
	outval = dot(color.rgb, vec3(0.2126, 0.7152, 0.0722));
#else
	outval = dot(color.rgb, vec3(0.35, 0.45, 0.2)); /* keep these factors in sync with texture.h:RGBTOBW */
#endif
}

void invert(float fac, vec4 col, out vec4 outcol)
{
	outcol.xyz = mix(col.xyz, 1.0 - col.xyz, fac);
	outcol.w = col.w;
}
...
{
	color = texture2D(ima, vec.xy * 0.5 + 0.5, lodbias);
	value = color.a;
...
{
	incol = mix(outcol, texcol * outcol, fact * facg);
}
...
{
    if (refrac < 1.0 || spec_power == 0.0) {
        specfac = 0.0;
        return;
    }

    spec_power = spec_power < 100.0 ? sqrt(1.0 / spec_power) : 10.0 / spec_power;
    ...
    float fres = pow((p - vh) / (p + vh), 2.0);
    fres *= 1.0 + pow(((vh * (p + vh) - 1.0) / (vh * (p - vh) + 1.0)), 2.0);

    float gaussian = exp(-pow(1.0 - nh, 2.0) / (2.0 * spec_power * spec_power));
    specfac = max(fres * g * gaussian, 0.0);
}
