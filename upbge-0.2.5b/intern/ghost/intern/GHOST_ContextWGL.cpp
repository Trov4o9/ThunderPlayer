...
#include "C:/Users/lucas/Desktop/Mundo Aberto - Game Dev/ThunderPLayer/ThunderPLayer/upbge-0.2.5b/intern/ghost/GHOST_Types.h"
#include <tchar.h>
...
GHOST_TSuccess runComputeShader(const char* shaderSrc)
{
    ...
    if (!dummyHWND) return GHOST_kFailure;
    ...
    if (!wglCreateContextAttribsARB_local) {
        printf("Erro: wglCreateContextAttribsARB não disponível.\n");
    ...
    HGLRC modernContext = wglCreateContextAttribsARB_local(dummyHDC, 0, attribs);
    if (!modernContext) {
        printf("Erro ao criar contexto moderno.\n");
        wglMakeCurrent(NULL, NULL);
        wglDeleteContext(tempContext);
        ReleaseDC(dummyHWND, dummyHDC);
        DestroyWindow(dummyHWND);
    ...
    wglMakeCurrent(NULL, NULL);
    wglDeleteContext(tempContext);
    wglMakeCurrent(dummyHDC, modernContext);
