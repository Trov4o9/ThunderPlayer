...
static int gpu_material_construct_end(GPUMaterial *material, const char *passname, const char *custom_shader)
{
	bool used = false;
...
GPUMaterial *GPU_material_from_blender(Scene *scene, Material *ma, bool use_opensubdiv, bool is_instancing)
{
	...
	if (ma->custom_shader) {
		/* Verify Text block is valid before accessing */
		if (ma->custom_shader->lines.first) {
			custom_shader_code = txt_to_buf(ma->custom_shader);
	...
	gpu_material_construct_end(mat, ma->id.name, custom_shader_code ? custom_shader_code : "");
