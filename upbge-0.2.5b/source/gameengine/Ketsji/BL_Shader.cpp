...
EXP_PYMETHODDEF_DOC(BL_Shader, setSource, " setSource(vertexProgram, fragmentProgram, computeProgram, apply)")
{
	if (m_shader) {
		Py_RETURN_NONE;
	}
	...
	if (PyArg_ParseTuple(args, "sssi:setSource", &v, &f, &c, &apply)) {
		...
		m_progs[COMPUTE_PROGRAM] = ""; // Novo: garantir que compute fique vazio
