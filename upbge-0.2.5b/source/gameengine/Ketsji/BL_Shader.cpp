...
EXP_PYMETHODDEF_DOC(BL_Shader, setSource, " setSource(vertexProgram, fragmentProgram, computeProgram, apply, apply2, force)")
{
    char* v;
    char* f;
    char* c;
    ...
    if (PyArg_ParseTuple(args, "sssiii:setSource", &v, &f, &c, &apply, &apply2, &force)) {
        
        m_progs[VERTEX_PROGRAM] = std::string(v);
        m_progs[FRAGMENT_PROGRAM] = std::string(f);
        m_progs[COMPUTE_PROGRAM] = std::string(c);
...
EXP_PYMETHODDEF_DOC(BL_Shader, setSourceList, " setSource(vertexProgram, fragmentProgram, computeProgram, apply")
{
	if (m_shader) {
		Py_RETURN_NONE;
	}
	...
	if (!PyArg_ParseTuple(args, "O!i:setSourceList", &PyDict_Type, &pydict, &apply, &apply2)) {
		return nullptr;
	}

	bool error = false;
	static const char *progname[MAX_PROGRAM] = { "vertex", "fragment", "geometry", "compute" };
	static const bool optional[MAX_PROGRAM] = { false, false, true, true };
