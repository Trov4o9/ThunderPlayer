...
#define THUNDER_DBVT_ENABLE_OPTIMIZED_KDOP 1
#define THUNDER_DBVT_ENABLE_OPENMP 1
#define THUNDER_DBVT_ENABLE_AVX2 1
#define THUNDER_DBVT_SOA_BATCH_SIZE 8
...
#if THUNDER_DBVT_ENABLE_AVX2
#include <immintrin.h>
#endif

#if THUNDER_DBVT_ENABLE_OPENMP
#include <omp.h>
#endif
...
struct LeafCollector : btDbvt::ICollide {
	std::vector<const btDbvtNode*> leaves;
...
struct DbvtLeafSoA {
	...
	std::vector<KX_ClientObjectInfo *> infos;
	...
	void push(const btDbvtNode *leaf, KX_ClientObjectInfo *info)
	{
		const btVector3& mins = leaf->volume.Mins();
		const btVector3& maxs = leaf->volume.Maxs();
		minX.push_back(mins.x());
		minY.push_back(mins.y());
		minZ.push_back(mins.z());
		maxX.push_back(maxs.x());
		maxY.push_back(maxs.y());
		maxZ.push_back(maxs.z());
...
static inline void CollectLeafSoA(const std::vector<const btDbvtNode *>& leaves, DbvtLeafSoA& soa)
{
	soa.reserve(leaves.size());
	for (const btDbvtNode *leaf : leaves) {
...
static inline bool LeafVisibleScalar(const DbvtLeafSoA& soa, const size_t index, const btVector3 *planes_n, const btScalar *planes_o)
{
	for (int plane = 0; plane < 6; ++plane) {
		const float px = (planes_n[plane].x() >= 0.0f) ? soa.minX[index] : soa.maxX[index];
		const float py = (planes_n[plane].y() >= 0.0f) ? soa.minY[index] : soa.maxY[index];
		const float pz = (planes_n[plane].z() >= 0.0f) ? soa.minZ[index] : soa.maxZ[index];
		const float distance = px * planes_n[plane].x() + py * planes_n[plane].y() + pz * planes_n[plane].z() + planes_o[plane];
		if (distance < 0.0f) {
			return false;
		}
	}
...
#if THUNDER_DBVT_ENABLE_AVX2
static inline unsigned DbvtVisibleMask8(const DbvtLeafSoA& soa, const size_t base, const btVector3 *planes_n, const btScalar *planes_o)
{
	__m256 rejected = _mm256_setzero_ps();
	const __m256 zero = _mm256_setzero_ps();

	for (int plane = 0; plane < 6; ++plane) {
		const __m256 px = _mm256_loadu_ps((planes_n[plane].x() >= 0.0f) ? &soa.minX[base] : &soa.maxX[base]);
		const __m256 py = _mm256_loadu_ps((planes_n[plane].y() >= 0.0f) ? &soa.minY[base] : &soa.maxY[base]);
		const __m256 pz = _mm256_loadu_ps((planes_n[plane].z() >= 0.0f) ? &soa.minZ[base] : &soa.maxZ[base]);

		__m256 distance = _mm256_mul_ps(px, _mm256_set1_ps(planes_n[plane].x()));
		distance = _mm256_add_ps(distance, _mm256_mul_ps(py, _mm256_set1_ps(planes_n[plane].y())));
		distance = _mm256_add_ps(distance, _mm256_mul_ps(pz, _mm256_set1_ps(planes_n[plane].z())));
		distance = _mm256_add_ps(distance, _mm256_set1_ps(planes_o[plane]));

		rejected = _mm256_or_ps(rejected, _mm256_cmp_ps(distance, zero, _CMP_LT_OQ));
	}

	return (unsigned)(~_mm256_movemask_ps(rejected)) & 0xffu;
}
#endif

static inline void FilterVisibleLeaves(const DbvtLeafSoA& soa, const btVector3 *planes_n, const btScalar *planes_o,
                                       std::vector<unsigned char>& visibleMask)
{
	const size_t count = soa.size();
	visibleMask.assign(count, 0);

#if THUNDER_DBVT_ENABLE_OPENMP
	const int max_threads = omp_get_num_procs() / 2;
	const int usable = (max_threads > 2) ? (max_threads - 3) : max_threads;
	const int omp_threads = (usable > 0) ? usable : 1;
	omp_set_num_threads(omp_threads);

	#pragma omp parallel for schedule(static) num_threads(omp_threads)
#endif
	for (int batchBase = 0; batchBase < (int)count; batchBase += THUNDER_DBVT_SOA_BATCH_SIZE) {
		const size_t base = (size_t)batchBase;
		const size_t remaining = count - base;
		const size_t batchCount = (remaining >= THUNDER_DBVT_SOA_BATCH_SIZE) ? THUNDER_DBVT_SOA_BATCH_SIZE : remaining;

#if THUNDER_DBVT_ENABLE_AVX2
		if (batchCount == THUNDER_DBVT_SOA_BATCH_SIZE) {
			const unsigned mask = DbvtVisibleMask8(soa, base, planes_n, planes_o);
			for (size_t lane = 0; lane < THUNDER_DBVT_SOA_BATCH_SIZE; ++lane) {
				visibleMask[base + lane] = (mask >> lane) & 1u;
			}
			continue;
		}
#endif
		...
		for (size_t lane = 0; lane < batchCount; ++lane) {
			visibleMask[base + lane] = LeafVisibleScalar(soa, base + lane, planes_n, planes_o) ? 1 : 0;
		}
	}
}
...
#if THUNDER_DBVT_ENABLE_OPTIMIZED_KDOP
	if (!occlusionRes) {
		LeafCollector collector;
		btDbvt::collideKDOP(m_cullingTree->m_sets[1].m_root, planes_n, planes_o, 6, collector);
		btDbvt::collideKDOP(m_cullingTree->m_sets[0].m_root, planes_n, planes_o, 6, collector);

		DbvtLeafSoA soa;
		CollectLeafSoA(collector.leaves, soa);

		std::vector<unsigned char> visibleMask;
		FilterVisibleLeaves(soa, planes_n, planes_o, visibleMask);

		for (size_t i = 0; i < soa.size(); ++i) {
			if (visibleMask[i]) {
				(*callback)(soa.infos[i], userData);
			}
		}
		return true;
	}
