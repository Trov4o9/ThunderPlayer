...
bool RAS_Shader::LinkProgram()
{
	...
	// Caso compute shader isolado (sem vertex/frag/geom)
	if (has_comp && !has_vert && !has_frag && !has_geom) {
		comp = GetParsedProgram(COMPUTE_PROGRAM);

		// Em vez de só criar, executa o compute shader usando sua função
		if (runComputeShader(comp.c_str()) != GHOST_kSuccess) {
			m_error = true;
			return false;
		}

		// Você pode decidir se ainda quer guardar algo aqui, por enquanto:
		m_shader = nullptr; // ou algum valor válido, dependendo do contexto
		m_error = false;
		return true;
	}

	// Caso shader gráfico tradicional (vertex/fragment obrigatórios)
