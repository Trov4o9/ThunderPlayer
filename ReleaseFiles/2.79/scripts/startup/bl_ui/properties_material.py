...
class MATERIAL_OT_refresh_shader(bpy.types.Operator):
    """Refresh the material shader (force recompilation)"""
    bl_idname = "material.refresh_shader"
...
class MATERIAL_OT_create_shader_example(bpy.types.Operator):
    """Create an example custom vertex shader"""
    bl_idname = "material.create_shader_example"
    bl_label = "Create Example Shader"
    bl_options = {'REGISTER', 'UNDO'}
...
"""
        ...
        if context.material:
            context.material.custom_shader = text
            self.report({'INFO'}, f"Created example shader: {text.name}")
        else:
...
class MATERIAL_OT_create_fragment_shader_example(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Add example code
        example_code = """// Custom Fragment Shader Example
// Global variables (outside functions)
float pulse_speed = 2.0;

// Fragment function - code here modifies material properties before lighting
void fragment() {
    // Available variables to modify:
    // ALBEDO - vec3 - Base color
    // ROUGHNESS - float - Surface roughness (0.0 to 1.0)
    // METALLIC - float - Metallic property (0.0 to 1.0)
    // EMIT - float - Emission strength
    // EMIT_RGB - vec3 - Emission color
    // SPECULAR - float - Specular intensity
    // SPECULAR_RGB - vec3 - Specular color
    // ALPHA - float - Transparency
    // NORMAL - vec3 - Surface normal
    // VERTEX - vec3 - Fragment position in view space
    // VIEW - vec3 - View direction
    // TIME - float - Animated time value
    // OBJECT_COLOR - vec4 - Object color
    
    // Example 1: Pulsating emission
    float pulse = (sin(TIME * pulse_speed) * 0.5 + 0.5);
    EMIT = pulse;
    EMIT_RGB = vec3(1.0, 0.5, 0.0) * pulse;
    
    // Example 2: Gradient based on position
    float gradient = VERTEX.z * 0.1;
    ALBEDO = mix(ALBEDO, vec3(0.0, 1.0, 1.0), gradient);
    
    // Example 3: Modify roughness based on view angle
    float fresnel = pow(1.0 - dot(NORMAL, VIEW), 3.0);
    ROUGHNESS = mix(ROUGHNESS, 0.1, fresnel);
}
"""
        text.write(example_code)
