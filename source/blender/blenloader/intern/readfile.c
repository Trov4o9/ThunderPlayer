...
static void lib_link_material(FileData *fd, Main *main)
{
	for (Material *ma = main->mat.first; ma; ma = ma->id.next) {
		if (ma->id.tag & LIB_TAG_NEED_LINK) {
			...
			/* Link custom shader Text data-block */
			ma->custom_shader = newlibadr_us(fd, ma->id.lib, ma->custom_shader);
			
			/* Link custom fragment shader Text data-block */
			ma->custom_fragment_shader = newlibadr_us(fd, ma->id.lib, ma->custom_fragment_shader);
...
static void direct_link_material(FileData *fd, Material *ma)
{
	...
	/* Custom shader is an ID pointer, will be resolved in lib_link_material */
	/* Do NOT use newdataadr here, it's handled by lib_link */
...
static void expand_material(FileData *fd, Main *mainvar, Material *ma)
{
	...
	/* Expand custom shader Text data-block */
	if (ma->custom_shader)
		expand_doit(fd, mainvar, ma->custom_shader);
	
	/* Expand custom fragment shader Text data-block */
	if (ma->custom_fragment_shader)
		expand_doit(fd, mainvar, ma->custom_fragment_shader);
}
