...
struct GPUTexture {
	...
	GLuint64 bindlessHandle;    /* ARB_bindless_texture handle (0 = not created) */
	int bindlessResident;       /* non-zero if handle is currently resident */
};
...
GPUTexture *GPU_texture_create_3D(int w, int h, int depth, int channels, const float *fpixels)
{
	...
	if (rescale && fpixels) {
		...
		glTexImage3D(tex->target, 0, internalformat, tex->w, tex->h, tex->depth, 0, format, type, pixels);
	}
...
GPUTexture *GPU_texture_create_2D_array(int w, int h, int depth, int channels, const float *fpixels)
{
	GLenum type, format, internalformat;
	void *pixels = NULL;

	GPUTexture *tex = MEM_callocN(sizeof(GPUTexture), "GPUTextureArray");
	tex->w = w;
	tex->h = h;
	tex->depth = depth;
	tex->number = -1;
	...
	GPU_ASSERT_NO_GL_ERRORS("2D_ARRAY glBindTexture");
	...
	glTexImage3D(tex->target, 0, internalformat, tex->w, tex->h, tex->depth, 0, format, GL_UNSIGNED_BYTE, pixels);
...
unsigned long long GPU_texture_bindless_handle(const GPUTexture *tex)
{
	if (!tex) return 0;
	return (unsigned long long)tex->bindlessHandle;
}

int GPU_texture_make_bindless_resident(GPUTexture *tex)
{
	if (!tex) return 0;
	if (!GLEW_ARB_bindless_texture) return 0;
	if (tex->bindlessResident) return 1; /* idempotent */
...
void GPU_texture_invalidate_bindless(GPUTexture *tex)
{
	if (!tex) return;
	if (tex->bindlessResident && tex->bindlessHandle) {
...
/* DSA-based filter update: uses glTextureParameteri (GL 4.5 / ARB_direct_state_access)
 * so it works without binding the texture to a unit — required in bindless mode. */
void GPU_texture_filter_mode_dsa(GPUTexture *tex, int compare, int use_filter, int mipmap)
{
	if (!tex || !tex->bindcode) return;

	GLuint id = tex->bindcode;
	int target = tex->target_base;

	if (tex->depth) {
		glTextureParameteri(id, GL_TEXTURE_COMPARE_MODE,
		                    compare ? GL_COMPARE_R_TO_TEXTURE : GL_NONE);
	}
	...
	/* If the handle is already resident, it must be made non-resident before
	 * changing sampler state, then re-made resident. */
	if (tex->bindlessResident && tex->bindlessHandle) {
		glMakeTextureHandleNonResidentARB(tex->bindlessHandle);
		tex->bindlessHandle = 0;
		tex->bindlessResident = 0;
...
void GPU_texture_filter_mode(GPUTexture *tex, bool compare, bool use_filter, bool mipmap)
{
	...
	/* In bindless mode tex->number is always -1; use the DSA path instead. */
	if (tex->number == -1) {
		GPU_texture_filter_mode_dsa(tex, compare ? 1 : 0, use_filter ? 1 : 0, mipmap ? 1 : 0);
		return;
	}
...
void GPU_texture_free(GPUTexture *tex)
{
	...
	if (tex->refcount == 0) {
		...
		/* Make handle non-resident before deleting (must happen even when fromblender=1) */
		if (tex->bindlessResident && tex->bindlessHandle) {
			glMakeTextureHandleNonResidentARB(tex->bindlessHandle);
			tex->bindlessHandle = 0;
			tex->bindlessResident = 0;
