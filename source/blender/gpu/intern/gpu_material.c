...
const char *gpu_shader_atmospheric_scattering_glsl = 
...
"  \n"
"  float zenithDensity(float x, float d) { \n"
"      return d / pow(max(x, 0.35e-2), 0.75); \n"
"  } \n"
"  \n"
...
"  } \n"
"  \n"
"  float getSunPoint(vec3 pos, vec3 light_dir) { \n"
"      return smoothstep(0.03, 0.026, distance(pos, light_dir)) * 50.0; \n"
"  } \n"
"  \n"
"  float getRayleigMultiplier(vec3 pos, vec3 light_dir) { \n"
"      return 1.0 + pow(1.0 - clamp(distance(pos, light_dir), 0.0, 1.0), 2.0) * pi * 0.5; \n"
"  } \n"
"  \n"
"  float getMie(vec3 pos, vec3 light_dir) { \n"
"      float disk = clamp(1.0 - pow(distance(pos, light_dir), 0.1), 0.0, 1.0); \n"
"      \n"
"      return disk * disk * (3.0 - 2.0 * disk) * 2.0 * pi; \n"
"  } \n"
"  \n"
"  vec3 getAtmosphericScattering(vec3 pos, vec3 light_dir, vec3 sky_color, float density, float scatter) { \n"
"      float zenith = zenithDensity(pos.z, density); \n"
"      float sunPointDistMult =  clamp(length(max(light_dir.z + scatter, 0.0)), 0.0, 1.0); \n"
"  \n"
"      float rayleighMult = getRayleigMultiplier(pos, light_dir); \n"
"  \n"
"      vec3 absorption    = getSkyAbsorption(sky_color, zenith); \n"
"      vec3 sunAbsorption = getSkyAbsorption(sky_color, zenithDensity(light_dir.z + scatter, density)); \n"
"  \n"
"      vec3 sky = sky_color * zenith * rayleighMult; \n"
"      vec3 sun = vec3(getSunPoint(pos, light_dir)); \n"
"      vec3 mie = getMie(pos, light_dir) * sunAbsorption; \n"
"  \n"
"      vec3 totalSky = mix(sky * absorption, sky / (sky + vec3(0.5)), sunPointDistMult); \n"
"           totalSky += (sun * 10.0) + mie; \n"
"           totalSky *= sunAbsorption * 0.5 + vec3(0.5 * length(sunAbsorption)); \n"
"  \n"
"      return totalSky / pi; \n"
"  } \n"
"  \n"
"  void node_atmospheric_scattering(vec3 world_pos, vec3 horizon_col, vec3 zenith_col, float env_energy, out vec4 outcol) { \n"
"      vec3 view_dir = normalize(world_pos); \n"
"      vec3 sun_pos = u_SunPosition; \n"
"      if (length(sun_pos) < 0.001) sun_pos = vec3(0.0, 0.0, 1.0); \n"
"      vec3 light_dir = normalize(sun_pos); \n"
"      float density = 0.7; \n"
"      float scatter = 0.2; \n"
"      float factor = clamp(view_dir.z, 0.0, 1.0); \n"
"      vec3 sky_color = mix(horizon_col, zenith_col, factor); \n"
"      vec3 col = getAtmosphericScattering(view_dir, light_dir, sky_color, density, scatter); \n"
"      outcol = vec4(col, 1.0); \n"
"  } \n";
...
struct GPUMaterial {
	...
	int timeloc;
	int custom_time_loc;  /* For custom shader unftime uniform */
	...
	char *custom_shader_code;           /* String do custom shader original */
	...
	GPUShader *shadow_shader_black_instancing;
	GPUShader *shadow_shader_vsm;
	GPUShader *shadow_shader_vsm_instancing;
	int shadow_time_loc;                /* Location de unftime nos shadow shaders */
};
...
static GPUMaterial *GPU_material_construct_begin(Material *ma)
{
	...
	material->ma = ma;
	material->custom_time_loc = -1;  /* Initialize to invalid */
	
	/* Initialize custom shader fields */
	material->custom_shader_code = NULL;
	...
	material->shadow_shader_black_instancing = NULL;
	material->shadow_shader_vsm = NULL;
	material->shadow_shader_vsm_instancing = NULL;
	material->shadow_time_loc = -1;
...
static int gpu_material_construct_end(GPUMaterial *material, const char *passname, const char *custom_shader, const char *custom_fragment_shader)
{
    bool used = false;
    ...
    if (used) {
        ...
        /* Parse custom shader if it exists */
        if (custom_shader && custom_shader[0] != '\0') {
            /* Store original shader code */
            material->custom_shader_code = BLI_strdup(custom_shader);
            
            /* Parse to extract vertex, global and fragment code */
            char *fragment_code = NULL;  /* We don't need fragment code for shadows */
            GPU_parse_custom_shader(custom_shader, &material->custom_global_code, &material->custom_vertex_code, &fragment_code);
            
            /* Free fragment code as we don't use it for shadows */
            if (fragment_code) {
                MEM_freeN(fragment_code);
            }
            
            /* Set flag if vertex code was extracted */
            material->has_custom_vertex_shader = (material->custom_vertex_code != NULL);
        }
...
void GPU_material_free(ListBase *gpumaterial)
{
	for (LinkData *link = gpumaterial->first; link; link = link->next) {
		...
		/* Free custom shader strings */
		if (material->custom_shader_code) {
			MEM_freeN(material->custom_shader_code);
		}
		if (material->custom_vertex_code) {
			MEM_freeN(material->custom_vertex_code);
		}
		if (material->custom_global_code) {
			MEM_freeN(material->custom_global_code);
		}
		
		/* Free shadow shaders (only if they're not builtin shared shaders) */
		/* Note: builtin shaders are managed globally, custom ones are per-material */
		if (material->has_custom_vertex_shader) {
			if (material->shadow_shader_black) {
				GPU_shader_free(material->shadow_shader_black);
			}
			if (material->shadow_shader_black_instancing) {
				GPU_shader_free(material->shadow_shader_black_instancing);
			}
			if (material->shadow_shader_vsm) {
				GPU_shader_free(material->shadow_shader_vsm);
			}
			if (material->shadow_shader_vsm_instancing) {
				GPU_shader_free(material->shadow_shader_vsm_instancing);
			}
		}
...
/* Custom shader support - getters for shadow shader system */
bool GPU_material_has_custom_vertex_shader(GPUMaterial *material)
{
	return material ? material->has_custom_vertex_shader : false;
}

const char *GPU_material_get_custom_vertex_code(GPUMaterial *material)
{
	return material ? material->custom_vertex_code : NULL;
}

const char *GPU_material_get_custom_global_code(GPUMaterial *material)
{
	return material ? material->custom_global_code : NULL;
}

GPUShader **GPU_material_get_shadow_shader_cache(GPUMaterial *material, int shadow_type)
{
	if (!material) return NULL;
	
	switch (shadow_type) {
		case 0: return &material->shadow_shader_black;
		case 1: return &material->shadow_shader_black_instancing;
		case 2: return &material->shadow_shader_vsm;
		case 3: return &material->shadow_shader_vsm_instancing;
		default: return NULL;
	}
}

int *GPU_material_get_shadow_time_loc(GPUMaterial *material)
{
	return material ? &material->shadow_time_loc : NULL;
}

Material *GPU_material_get_material(GPUMaterial *material)
{
	return material ? material->ma : NULL;
}
        ...
        float viewmat[4][4], float viewinv[4][4], float camerafactors[4], bool scenelock)
{
	if (material->pass) {
		...
		/* Bind custom time uniform if it exists */
		if (material->custom_time_loc >= 0) {
			float ftime = (float)time;
		 ...
		 * For game-engine materials using MA_UBO_LIGHTING the game-engine's
		 * RAS_LightManager UBO will be rebound immediately after this call
		 * (in BL_BlenderShader::BindProg), so the viewport value here is a
		 * short-lived placeholder.  For viewport rendering this IS the final bind. */
        ...
        float autobumpscale, GPUParticleInfo *pi, float object_info[3])
{
	if (material->pass) {
		...
		if (material->ma) {
			...
			//int loc_alp = glGetUniformLocation(shader->program, "u_MatAlpha");
			//int loc_col = glGetUniformLocation(shader->program, "u_MatColor");
			//int loc_spec = glGetUniformLocation(shader->program, "u_MatSpec");
			//int loc_rough = glGetUniformLocation(shader->program, "u_MatRoughness");
			//int loc_metal = glGetUniformLocation(shader->program, "u_MatMetallic");
			//int loc_emit = glGetUniformLocation(shader->program, "u_MatEmission");

			//if (loc_col != -1) glUniform3f(loc_col, ma->r, ma->g, ma->b);
			//if (loc_alp != -1) glUniform1f(loc_alp, material->alpha);
			//if (loc_spec != -1) glUniform1f(loc_spec, ma->spec);
			//if (loc_rough != -1) glUniform1f(loc_rough, ma->roughness_bsdf);
			//if (loc_metal != -1) glUniform1f(loc_metal, ma->metallic_bsdf);
			//if (loc_emit != -1) glUniform1f(loc_emit, ma->emit);

			//printf("Material Diffuse: R=%.3f G=%.3f B=%.3f | Spec=%.3f | Rough=%.3f | Metal=%.3f | Emit=%.3f\n",
			//	ma->r, ma->g, ma->b,
			//	ma->spec, ma->roughness_bsdf,
			//	ma->metallic_bsdf, ma->emit);
		}
...
static void shade_one_light(GPUShadeInput *shi, GPUShadeResult *shr, GPULamp *lamp)
{
    ...
    if (g_useDeferred_GG) {
        printf("[shade_one_light] g_useDeferred_GG = %d → ignorando luzes GLSL\n", g_useDeferred_GG);
        return;
    }
...
void GPU_shadeinput_set(GPUMaterial *mat, Material *ma, GPUShadeInput *shi)
{
	...
	/* UBO PBR overrides — always linked so the cons survive prune and get tracked.
	 * ubo_spec_color defaults to (0.04,0.04,0.04); ubo_spec_strength defaults to 1.0 via
	 * the DNA field zero-init + versioning.  New fields default to 0 in old .blends,
	 * but 0 spec_strength would kill specular, so clamp is handled in the shader. */
	GPU_link(mat, "set_rgb",   GPU_select_uniform(ma->ubo_spec_color,     GPU_DYNAMIC_MAT_UBO_SPECCOLOR,    ma, ma), &shi->ubo_spec_color);
	GPU_link(mat, "set_value", GPU_select_uniform(&ma->ubo_spec_strength,  GPU_DYNAMIC_MAT_UBO_SPECSTRENGTH, ma, ma), &shi->ubo_spec_strength);
	GPU_link(mat, "set_rgb",   GPU_select_uniform(ma->ubo_scatter_color,  GPU_DYNAMIC_MAT_UBO_SCATTERCOLOR, ma, ma), &shi->ubo_scatter_color);
	GPU_link(mat, "set_value", GPU_select_uniform(&ma->ubo_scatter_fac,   GPU_DYNAMIC_MAT_UBO_SCATTERFAC,   ma, ma), &shi->ubo_scatter_fac);
...
void GPU_shaderesult_set(GPUShadeInput *shi, GPUShadeResult *shr)
{
		...
		g_useDeferred_GG)
	{
		...
		if (ma->mode2 & MA_UBO_LIGHTING) {
			/* UBO lighting is injected by gpu_codegen.c (codegen_call_functions).
			 * ubo_lighting_apply is a stub node whose call is replaced entirely by
			 * the codegen with:  tmp_out = tmp_in.rgb + ubo_result.
			 * All extra inputs are declared only so their cons nodes survive
			 * gpu_nodes_prune; their IDs are tracked for ROUGHNESS/METALLIC/
			 * SCATTER_RGB/SCATTER_FAC variables in main(). */
			GPU_link(mat, "ubo_lighting_apply", shr->diff,
			         shi->roughness_bsdf, shi->metallic_bsdf,
			         shi->ubo_spec_color, shi->ubo_spec_strength,
			         shi->ubo_scatter_color, shi->ubo_scatter_fac,
			         &shr->diff);
		}
		else {
...
static void gpu_material_old_world(struct GPUMaterial *mat, struct World *wo)
{
	...
	if ((wo->skytype & (WO_SKYBLEND + WO_SKYTEX)) == 0) {
#if UPBGE_ATMOSPHERIC_SCATTERING
		GPUNodeLink *hor = GPU_dynamic_uniform(&wo->horr, GPU_DYNAMIC_HORIZON_COLOR, NULL);
		GPUNodeLink *zen = GPU_dynamic_uniform(&wo->zenr, GPU_DYNAMIC_ZENITH_COLOR, NULL);
		GPUNodeLink *env = GPU_dynamic_uniform(&GPUWorld.envlightenergy, GPU_DYNAMIC_ENVLIGHT_ENERGY, NULL);
		GPUNodeLink *pos = GPU_material_builtin(mat, GPU_VIEW_POSITION);
		GPU_link(mat, "node_atmospheric_scattering", pos, hor, zen, env, &shr.combined);
		
		/* exposure correction */
		if (wo->exp != 0.0f || wo->range != 1.0f) {
			GPU_link(mat, "shade_exposure_correct", shr.combined,
				GPU_dynamic_uniform(&GPUWorld.linfac, GPU_DYNAMIC_WORLD_LINFAC, NULL),
				GPU_dynamic_uniform(&GPUWorld.logfac, GPU_DYNAMIC_WORLD_LOGFAC, NULL),
				&shr.combined);
		}
#else
		...
		{
			GPUNodeLink *env = GPU_dynamic_uniform(&GPUWorld.envlightenergy, GPU_DYNAMIC_ENVLIGHT_ENERGY, NULL);
			GPU_link(mat, "node_atmospheric_scattering", ray, hor, zen, env, &shi.rgb);
			
			/* exposure correction */
			if (wo->exp != 0.0f || wo->range != 1.0f) {
				GPU_link(mat, "shade_exposure_correct", shi.rgb,
					GPU_dynamic_uniform(&GPUWorld.linfac, GPU_DYNAMIC_WORLD_LINFAC, NULL),
					GPU_dynamic_uniform(&GPUWorld.logfac, GPU_DYNAMIC_WORLD_LOGFAC, NULL),
					&shi.rgb);
			}
...
GPUMaterial *GPU_material_world(Scene *scene, World *wo)
{
    LinkData *link;
    GPUMaterial *mat;
    ...
    /* Store sky mode in GPUMaterial so GPU_generate_pass() can read it */
    mat->sky_shader_mode = (custom_sky_code != NULL) ? wo->sky_shader_mode : WO_SKY_MODE_PREPROCESS;
...
GPUMaterial *GPU_material_from_blender(Scene *scene, Material *ma, bool use_opensubdiv, bool is_instancing)
{
	...
	/* Get custom shader code from Text data-block */
    char *custom_shader_code = NULL;
    if (ma->custom_shader) {
        /* Validate Text pointer before accessing */
        Text *text = ma->custom_shader;
        
        /* Check if pointer is valid and Text data is loaded */
        if (text && (text->id.tag & LIB_TAG_DOIT) == 0) {
            /* Verify Text block has valid data */
            if (text->lines.first && text->curl && text->sell) {
                custom_shader_code = txt_to_buf(text);
                
                if (custom_shader_code) {
                    /* Force TIME builtin to be included when custom shader is used */
                    GPUNodeLink *timelink = GPU_builtin(GPU_TIME);
                    (void)timelink; /* Suppress unused warning */
                }
            }
        }
        else {
            /* Text block not loaded or invalid - clear the reference */
            ma->custom_shader = NULL;
        }
    }
    
    /* Get custom fragment shader */
    char *custom_fragment_shader_code = NULL;
    if (ma->custom_fragment_shader) {
        Text *text_frag = (Text *)ma->custom_fragment_shader;
        if (text_frag && (text_frag->id.tag & LIB_TAG_DOIT) == 0) {
            if (text_frag->lines.first && text_frag->curl && text_frag->sell) {
                custom_fragment_shader_code = txt_to_buf(text_frag);
                
                if (custom_fragment_shader_code) {
                    /* Force TIME builtin to be included when custom fragment shader is used */
                    GPUNodeLink *timelink = GPU_builtin(GPU_TIME);
                    (void)timelink;
                }
            }
            ...
            ma->custom_fragment_shader = NULL;
        }
    }
    
    gpu_material_construct_end(mat, ma->id.name, 
        custom_shader_code ? custom_shader_code : "",
        custom_fragment_shader_code ? custom_fragment_shader_code : "");
    
    /* Free the allocated buffers */
    if (custom_shader_code) {
        MEM_freeN(custom_shader_code);
    }
    if (custom_fragment_shader_code) {
        MEM_freeN(custom_fragment_shader_code);
    }
