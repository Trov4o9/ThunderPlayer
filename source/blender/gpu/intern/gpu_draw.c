...
static struct GPUTextureState {
	...
	int gpu_mipmap;
	MTexPoly *lasttface;
} GTS = {0, 0, 0, 0, 0, 0, 0, 0, NULL, NULL, 1, GPU_MIPMAP_FILTER_LINEAR_MIPMAP_NEAREST, 0, -1, 1.0f, 0, NULL};
...
{
	unsigned char newfilter = GTS.mipmapfilter;
	newfilter |= GPU_MIPMAP_FILTER_LINEAR_MIPMAP_NEAREST;
	newfilter = (newfilter & ~1) | (linear ? 1 : 0);
	GTS.mipmapfilter = newfilter;
}
...
{
	return (GTS.mipmapfilter & 1) != 0;
}

void GPU_set_mipmap_filter(GPU_MipmapFilter filter)
{
	GTS.mipmapfilter = (unsigned char)(filter & 3);
}

GPU_MipmapFilter GPU_get_mipmap_filter(void)
{
	return (GPU_MipmapFilter)(GTS.mipmapfilter & 3);
}
...
static GLenum gpu_get_mipmap_filter(bool mag)
{
	if (mag) {
		if (GTS.domipmap) {
			return (GTS.mipmapfilter & 2) ? GL_LINEAR : GL_NEAREST;
		}
		return GL_NEAREST;
	}
	else {
		if (GTS.domipmap) {
			switch (GTS.mipmapfilter & 3) {
				case GPU_MIPMAP_FILTER_NEAREST_MIPMAP_NEAREST:
					return GL_NEAREST_MIPMAP_NEAREST;
				case GPU_MIPMAP_FILTER_NEAREST_MIPMAP_LINEAR:
					return GL_NEAREST_MIPMAP_LINEAR;
				case GPU_MIPMAP_FILTER_LINEAR_MIPMAP_NEAREST:
					return GL_LINEAR_MIPMAP_NEAREST;
				case GPU_MIPMAP_FILTER_LINEAR_MIPMAP_LINEAR:
				default:
        ...
        int textarget, bool mipmap, bool use_high_bit_depth, Image *ima)
{
    ...
    if (textarget == GL_TEXTURE_2D) {
        if (use_high_bit_depth)
            glTexImage2D(GL_TEXTURE_2D, 0, GLEW_ARB_texture_float ? GL_RGBA16F_ARB : GL_RGBA16,
        ...
        if (GPU_get_mipmap() && mipmap) {
            ...
            if (ima) ima->tpageflag |= IMA_MIPMAP_COMPLETE;
        } else {
        ...
        if ((use_high_bit_depth && frect == NULL) || (!use_high_bit_depth && rect == NULL) || w != h) {
            ...
            if (GPU_get_mipmap() && mipmap) glGenerateMipmap(GL_TEXTURE_CUBE_MAP);
            glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, gpu_get_mipmap_filter(0));
            glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
            glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
            glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
            if (ima && GPU_get_mipmap()) ima->tpageflag |= IMA_MIPMAP_COMPLETE;
        }
    }
...
int GPU_object_material_bind(int nr, void *attribs)
{
	...
	if (GMS.lastretval) {
		...
		if (gattribs && GMS.gmatbuf[nr]) {
			...
			/* Use scene time for viewport animation */
			double scene_time = 1.0;
			if (GMS.gscene) {
				scene_time = BKE_scene_frame_get(GMS.gscene);
			}
			
			GPU_material_bind(
			        gpumat, GMS.glay, scene_time, !(GMS.gob->mode & OB_MODE_TEXTURE_PAINT),
