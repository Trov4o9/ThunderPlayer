...
static const char *gpu_shader_version_core(void)
{
    if (GLEW_VERSION_4_5) {
...
static void gpu_shader_standard_extensions(char defines[MAX_EXT_DEFINE_LENGTH], bool use_geometry_shader)
{
    ...
    if (GLEW_ARB_texture_query_lod) {
        ...
        strcat(defines, "#extension GL_ARB_tessellation_shader: enable\n");
    }
    ...
    /* Bindless textures: inject once, covers all shaders compiled via GPU_shader_create_ex */
    if (GLEW_ARB_bindless_texture &&
        !strstr(defines, "GL_ARB_bindless_texture"))
    {
        strcat(defines, "#extension GL_ARB_bindless_texture : require\n");
    }
}
...
GPUShader *GPU_shader_create_tess(const char *vertexcode,
                             const char *fragcode,
                             const char *geocode,
                             const char *libcode,
                             const char *defines,
                             ...
                             int number)
{
	return GPU_shader_create_ex_tess(vertexcode,
	                            fragcode,
	                            geocode,
	                            libcode,
	                            defines,
...
GPUShader *GPU_shader_create_ex_tess(const char *vertexcode,
                                const char *fragcode,
                                const char *geocode,
                                const char *libcode,
                                const char *defines,
...

{
	...
	if (vertexcode) {
		...
		source[num_source++] = gpu_shader_version_core();
...
/* -------------------------------------------------------------------- */
/** \name Material Shadow Shader System
 * \{ */

/* Helper: Get base builtin shadow shader for a given shadow type */
static GPUShader *GPU_shader_get_builtin_shader_for_shadow(GPUShadowShaderType shadow_type)
{
	switch (shadow_type) {
		case GPU_SHADOW_SHADER_BLACK:
			return GPU_shader_get_builtin_shader(GPU_SHADER_BLACK);
		case GPU_SHADOW_SHADER_BLACK_INSTANCING:
			return GPU_shader_get_builtin_shader(GPU_SHADER_BLACK_INSTANCING);
		case GPU_SHADOW_SHADER_VSM:
			return GPU_shader_get_builtin_shader(GPU_SHADER_VSM_STORE);
		case GPU_SHADOW_SHADER_VSM_INSTANCING:
			return GPU_shader_get_builtin_shader(GPU_SHADER_VSM_STORE_INSTANCING);
		default:
			return NULL;
	}
}

/* Helper: Create shadow shader with custom vertex code injected */
static GPUShader *gpu_create_shadow_shader_with_custom(
        const char *custom_vertex_code,
        const char *custom_global_code,
        GPUShadowShaderType shadow_type)
{
	if (!custom_vertex_code) {
		return NULL;  /* No custom code, caller should use builtin */
	}

	/* Determine which shadow shader template to use */
	const char *base_frag_code;
	const char *shader_defines = NULL;
	bool use_instancing = false;

	switch (shadow_type) {
		case GPU_SHADOW_SHADER_BLACK:
			base_frag_code = datatoc_gpu_shader_black_frag_glsl;
			break;
		case GPU_SHADOW_SHADER_BLACK_INSTANCING:
			base_frag_code = datatoc_gpu_shader_black_frag_glsl;
			shader_defines = "#define USE_INSTANCING;\n";
			use_instancing = true;
			break;
		case GPU_SHADOW_SHADER_VSM:
			base_frag_code = datatoc_gpu_shader_vsm_store_frag_glsl;
			break;
		case GPU_SHADOW_SHADER_VSM_INSTANCING:
			base_frag_code = datatoc_gpu_shader_vsm_store_frag_glsl;
			shader_defines = "#define USE_INSTANCING;\n";
	...
	/* Build modified vertex shader from scratch */
	/* Strategy: Don't use base shader at all - rebuild it with custom code like normal shaders do */
	
	char modified_vert[16384];  /* Large buffer for complete shader */
	modified_vert[0] = '\0';
	
	/* Add varyings needed by fragment shader */
	bool is_vsm = (shadow_type == GPU_SHADOW_SHADER_VSM || shadow_type == GPU_SHADOW_SHADER_VSM_INSTANCING);
	if (is_vsm) {
		strcat(modified_vert, "varying vec4 v_position;\n\n");
	}
	
	/* Add instancing inputs if needed */
	if (use_instancing) {
		strcat(modified_vert, "#ifdef USE_INSTANCING\n");
		strcat(modified_vert, "in mat3 ininstmatrix;\n");
		strcat(modified_vert, "in vec3 ininstposition;\n");
		strcat(modified_vert, "#endif\n\n");
	}
	
	/* Add global custom code */
	if (custom_global_code && strlen(custom_global_code) > 0) {
		strcat(modified_vert, "/* Custom Global Code */\n");
		strcat(modified_vert, custom_global_code);
		strcat(modified_vert, "\n\n");
	}
	
	/* Add uniform for time */
	strcat(modified_vert, "uniform float unftime;\n\n");
	
	/* Build main function with custom code */
	strcat(modified_vert, "void main()\n{\n");
	
	/* Define UPBGE custom shader variables */
	strcat(modified_vert, "\t/* UPBGE Custom Shader Variables */\n");
	strcat(modified_vert, "\tvec3 VERTEX = gl_Vertex.xyz;\n");
	strcat(modified_vert, "\tvec3 NORMAL = gl_Normal;\n");
	strcat(modified_vert, "\tfloat TIME = unftime;\n\n");
	
	/* Inject custom vertex code */
	strcat(modified_vert, "\t/* Custom Vertex Code */\n");
	/* Add tabs to each line of custom code */
	const char *line_start = custom_vertex_code;
	const char *line_end;
	while (*line_start) {
		strcat(modified_vert, "\t");
		line_end = strchr(line_start, '\n');
		if (line_end) {
			strncat(modified_vert, line_start, line_end - line_start + 1);
			line_start = line_end + 1;
		}
		else {
			strcat(modified_vert, line_start);
			break;
		}
	}
	strcat(modified_vert, "\n\n");
	
	/* Now calculate gl_Position using modified VERTEX */
	if (use_instancing) {
		strcat(modified_vert, "#ifdef USE_INSTANCING\n");
		strcat(modified_vert, "\tmat4 instmat = mat4(vec4(ininstmatrix[0], ininstposition.x),\n");
		strcat(modified_vert, "\t                    vec4(ininstmatrix[1], ininstposition.y),\n");
		strcat(modified_vert, "\t                    vec4(ininstmatrix[2], ininstposition.z),\n");
		strcat(modified_vert, "\t                    vec4(0.0, 0.0, 0.0, 1.0));\n\n");
		
		if (is_vsm) {
			strcat(modified_vert, "\tv_position = gl_ProjectionMatrix * gl_ModelViewMatrix * (vec4(VERTEX, 1.0) * instmat);\n");
			strcat(modified_vert, "\tgl_Position = v_position;\n");
		}
		else {
			strcat(modified_vert, "\tgl_Position = gl_ProjectionMatrix * gl_ModelViewMatrix * (vec4(VERTEX, 1.0) * instmat);\n");
		}
		
		strcat(modified_vert, "#else\n");
		
		if (is_vsm) {
			strcat(modified_vert, "\tgl_Position = gl_ProjectionMatrix * gl_ModelViewMatrix * vec4(VERTEX, 1.0);\n");
			strcat(modified_vert, "\tv_position = gl_Position;\n");
		}
		else {
			strcat(modified_vert, "\tgl_Position = gl_ProjectionMatrix * gl_ModelViewMatrix * vec4(VERTEX, 1.0);\n");
		}
	...
	/* Create shader with modified vertex code */
...
/* Main API: Get shadow shader for material (with custom code if needed) */
GPUShader *GPU_shader_get_material_shadow_shader(GPUMaterial *material, GPUShadowShaderType shadow_type)
{
	if (!material) {
		/* No material, use builtin */
		return GPU_shader_get_builtin_shader_for_shadow(shadow_type);
	}
	
	/* Check if material has custom vertex shader */
	if (!GPU_material_has_custom_vertex_shader(material)) {
		/* No custom shader, use builtin */
		return GPU_shader_get_builtin_shader_for_shadow(shadow_type);
	}
	
	/* Check cache for this shadow type */
	GPUShader **cache_slot = GPU_material_get_shadow_shader_cache(material, (int)shadow_type);
	if (!cache_slot) {
		return NULL;
	}
	...
	/* Create new shadow shader with custom code */
	GPUShader *shader = gpu_create_shadow_shader_with_custom(
	        GPU_material_get_custom_vertex_code(material),
	        GPU_material_get_custom_global_code(material),
	        shadow_type);
	...
	*cache_slot = shader;
	...
	/* Get TIME uniform location for shadow shader */
	int *time_loc = GPU_material_get_shadow_time_loc(material);
	if (time_loc) {
		*time_loc = GPU_shader_get_uniform(shader, "unftime");
	}
