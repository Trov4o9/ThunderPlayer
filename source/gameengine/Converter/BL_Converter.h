...
private:
	...
	std::vector<std::pair<KX_Scene *, KX_Mesh *> > m_delayedRemovedMeshes;
	...
	bool RemoveMeshNow(KX_Scene *scene, KX_Mesh *mesh);
	void ClearDelayedRemovedMeshes(KX_Scene *scene, KX_Mesh *mesh = nullptr);
...
public:
	...
	/// Schedule removal of a registered mesh object copy at the end of the frame.
	bool RemoveMesh(KX_Scene *scene, KX_Mesh *mesh);
	/// Effectively remove any mesh copies scheduled for deletion.
	void FlushRemovedMeshes();
