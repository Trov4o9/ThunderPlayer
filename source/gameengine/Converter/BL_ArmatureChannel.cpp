...
{
	m_has_base = false;
	m_pose_head_base[0] = m_pose_head_base[1] = m_pose_head_base[2] = 0.0f;
	m_pose_tail_base[0] = m_pose_tail_base[1] = m_pose_tail_base[2] = 0.0f;
}
...
PyAttributeDef BL_ArmatureChannel::AttributesPtr[] = {
	...
	EXP_PYATTRIBUTE_RO_FUNCTION("pose_head_base", BL_ArmatureChannel, py_attr_get_pose_head_base),
	EXP_PYATTRIBUTE_RO_FUNCTION("pose_tail_base", BL_ArmatureChannel, py_attr_get_pose_tail_base),
...
static PyObject *CreateMathutilsVector3(float x, float y, float z)
{
	PyObject *mathutils = PyImport_ImportModule("mathutils");
	if (!mathutils) {
		printf("[DBG] mathutils import failed, falling back to raw Vector_CreatePyObject\n");
		float v[3] = {x, y, z};
		return Vector_CreatePyObject(v, 3, nullptr);
	}
	PyObject *vec_class = PyObject_GetAttrString(mathutils, "Vector");
	if (!vec_class) {
		printf("[DBG] mathutils.Vector not found, fallback\n");
		Py_DECREF(mathutils);
		float v[3] = {x, y, z};
		return Vector_CreatePyObject(v, 3, nullptr);
	}
	PyObject *tuple3 = Py_BuildValue("(fff)", x, y, z);
	PyObject *args = PyTuple_Pack(1, tuple3);
	Py_DECREF(tuple3);
	PyObject *vec = PyObject_CallObject(vec_class, args);
	Py_DECREF(args);
	Py_DECREF(vec_class);
	Py_DECREF(mathutils);
	if (!vec) {
		printf("[DBG] mathutils.Vector call failed, fallback\n");
		float v[3] = {x, y, z};
		return Vector_CreatePyObject(v, 3, nullptr);
	}
	return vec;
...
PyObject *BL_ArmatureChannel::py_attr_get_pose_head_base(EXP_PyObjectPlus *self_v, const struct EXP_PYATTRIBUTE_DEF *attrdef)
{
	BL_ArmatureChannel *self = static_cast<BL_ArmatureChannel *>(self_v);
	float v[3];
	if (self->m_has_base) {
		v[0] = self->m_pose_head_base[0];
		v[1] = self->m_pose_head_base[1];
		v[2] = self->m_pose_head_base[2];
	}
	else {
...
PyObject *BL_ArmatureChannel::py_attr_get_pose_tail_base(EXP_PyObjectPlus *self_v, const struct EXP_PYATTRIBUTE_DEF *attrdef)
{
	BL_ArmatureChannel *self = static_cast<BL_ArmatureChannel *>(self_v);
	float v[3];
	if (self->m_has_base) {
		v[0] = self->m_pose_tail_base[0];
		v[1] = self->m_pose_tail_base[1];
		v[2] = self->m_pose_tail_base[2];
	}
	else {
		v[0] = 0.0f; v[1] = 0.0f; v[2] = 0.0f;
	}
