...
public:
	CM_AutoLock(CM_ThreadLock& lock)
		:m_lock(lock)
	{
		m_lock.Lock();
	}
...
CM_ThreadMutex& BL_ArmatureObject::GetPoseMutex()
{
	static CM_ThreadMutex mutex;
	return mutex;
}
...
void BL_ArmatureObject::ApplyPoseLocked()
{
	if (!m_objArma || !m_objArma->pose || !m_scene) {
		return;
	}
    ...
    if (m_lastapplyframe != m_lastframe) {
        ...
        // compute and cache base pose (without overrides)
        BKE_pose_where_is(m_scene, m_objArma);
        m_basePoseHead.clear();
        m_basePoseTail.clear();
        for (bPoseChannel *pchan = (bPoseChannel *)m_objArma->pose->chanbase.first; pchan; pchan = pchan->next) {
            m_basePoseHead[pchan->name] = {pchan->pose_head[0], pchan->pose_head[1], pchan->pose_head[2]};
            m_basePoseTail[pchan->name] = {pchan->pose_tail[0], pchan->pose_tail[1], pchan->pose_tail[2]};
        }
        // apply runtime overrides and recompute final pose
        ApplyBoneOverrides();
        BKE_pose_where_is(m_scene, m_objArma);
        // restore ourself
...
{
	if (!m_objArma || !m_objArma->data) {
		return ARM_VDEF_BLENDER;
	}
...
void BL_ArmatureObject::ApplyBoneOverrides()
{
    for (const auto& it : m_boneLocAdd) {
        bPoseChannel *pchan = BKE_pose_channel_find_name(m_objArma->pose, it.first.c_str());
        if (pchan) {
            pchan->loc[0] += it.second[0];
            pchan->loc[1] += it.second[1];
            pchan->loc[2] += it.second[2];
        }
    }
    for (const auto& it : m_boneEulAdd) {
        bPoseChannel *pchan = BKE_pose_channel_find_name(m_objArma->pose, it.first.c_str());
        if (pchan) {
            pchan->eul[0] += it.second[0];
            pchan->eul[1] += it.second[1];
            pchan->eul[2] += it.second[2];
        }
    }
    for (const auto& it : m_boneScaleMul) {
        bPoseChannel *pchan = BKE_pose_channel_find_name(m_objArma->pose, it.first.c_str());
        if (pchan) {
            pchan->size[0] *= it.second[0];
            pchan->size[1] *= it.second[1];
            pchan->size[2] *= it.second[2];
        }
    }
    for (const auto& it : m_boneQuatMul) {
        bPoseChannel *pchan = BKE_pose_channel_find_name(m_objArma->pose, it.first.c_str());
        if (pchan) {
            float mulq[4] = {it.second[0], it.second[1], it.second[2], it.second[3]};
            normalize_qt(mulq);
            if (pchan->rotmode == ROT_MODE_QUAT) {
                float base[4];
                copy_qt_qt(base, pchan->quat);
                normalize_qt(base);
                mul_qt_qtqt(pchan->quat, base, mulq);
                normalize_qt(pchan->quat);
            }
            else {
                float add_eul[3];
                quat_to_eulO(add_eul, pchan->rotmode, mulq);
                pchan->eul[0] += add_eul[0];
                pchan->eul[1] += add_eul[1];
                pchan->eul[2] += add_eul[2];
            }
        }
    }
}

void BL_ArmatureObject::SetBoneLocationAdd(const std::string& name, const mt::vec3& loc)
{
    m_boneLocAdd[name] = loc;
}

void BL_ArmatureObject::SetBoneEulerAdd(const std::string& name, const mt::vec3& eul)
{
    m_boneEulAdd[name] = eul;
}

void BL_ArmatureObject::SetBoneScaleMul(const std::string& name, const mt::vec3& scale)
{
    m_boneScaleMul[name] = scale;
}

void BL_ArmatureObject::SetBoneRotationMulQuat(const std::string& name, const mt::vec4& quat)
{
    m_boneQuatMul[name] = quat;
}

void BL_ArmatureObject::RemoveBoneOverride(const std::string& name)
{
    m_boneLocAdd.erase(name);
    m_boneEulAdd.erase(name);
    m_boneScaleMul.erase(name);
    m_boneQuatMul.erase(name);
}

void BL_ArmatureObject::ClearBoneOverrides()
{
    m_boneLocAdd.clear();
    m_boneEulAdd.clear();
    m_boneScaleMul.clear();
    m_boneQuatMul.clear();
}
...
PyMethodDef BL_ArmatureObject::Methods[] = {
    ...
    {"setBoneLocationAdd",      (PyCFunction)BL_ArmatureObject::py_setBoneLocationAdd, METH_VARARGS},
    {"setBoneEulerAdd",         (PyCFunction)BL_ArmatureObject::py_setBoneEulerAdd, METH_VARARGS},
    {"setBoneScaleMul",         (PyCFunction)BL_ArmatureObject::py_setBoneScaleMul, METH_VARARGS},
    {"setBoneRotationMulQuat",  (PyCFunction)BL_ArmatureObject::py_setBoneRotationMulQuat, METH_VARARGS},
    {"removeBoneOverride",      (PyCFunction)BL_ArmatureObject::py_removeBoneOverride, METH_VARARGS},
    {"clearBoneOverrides",      (PyCFunction)BL_ArmatureObject::py_clearBoneOverrides, METH_VARARGS},
    {NULL, NULL} // sentinel
};
...
PyObject *BL_ArmatureObject::py_setBoneLocationAdd(PyObject *self, PyObject *args)
{
    BL_ArmatureObject *arm =
        static_cast<BL_ArmatureObject*>(((EXP_PyObjectPlus_Proxy*)self)->ref);

    const char *name;
    float x, y, z;

    if (!PyArg_ParseTuple(args, "sfff:setBoneLocationAdd", &name, &x, &y, &z))
        Py_RETURN_NONE;

    arm->SetBoneLocationAdd(name, mt::vec3(x, y, z));
    Py_RETURN_NONE;
}



PyObject *BL_ArmatureObject::py_setBoneEulerAdd(PyObject *self, PyObject *args)
{
    BL_ArmatureObject *arm =
        static_cast<BL_ArmatureObject*>(((EXP_PyObjectPlus_Proxy*)self)->ref);
    ...
    if (!PyArg_ParseTuple(args, "sfff:setBoneEulerAdd", &name, &x, &y, &z))
        Py_RETURN_NONE;

    arm->SetBoneEulerAdd(name, mt::vec3(x, y, z));
    Py_RETURN_NONE;
}


PyObject *BL_ArmatureObject::py_setBoneScaleMul(PyObject *self, PyObject *args)
{
    BL_ArmatureObject *arm =
        static_cast<BL_ArmatureObject*>(((EXP_PyObjectPlus_Proxy*)self)->ref);
    const char *name;
    ...
    if (!PyArg_ParseTuple(args, "sfff:setBoneScaleMul", &name, &x, &y, &z))
        Py_RETURN_NONE;

    arm->SetBoneScaleMul(name, mt::vec3(x, y, z));
    Py_RETURN_NONE;
}


PyObject *BL_ArmatureObject::py_setBoneRotationMulQuat(PyObject *self, PyObject *args)
{
    BL_ArmatureObject *arm =
        static_cast<BL_ArmatureObject*>(((EXP_PyObjectPlus_Proxy*)self)->ref);
    const char *name;
    ...
    if (!PyArg_ParseTuple(args, "sffff:setBoneRotationMulQuat", &name, &x, &y, &z, &w))
        Py_RETURN_NONE;

    arm->SetBoneRotationMulQuat(name, mt::vec4(w, x, y, z));
    Py_RETURN_NONE;
}

PyObject *BL_ArmatureObject::py_removeBoneOverride(PyObject *self, PyObject *args)
{
    BL_ArmatureObject *arm =
        static_cast<BL_ArmatureObject*>(((EXP_PyObjectPlus_Proxy*)self)->ref);

    const char *name;
    if (!PyArg_ParseTuple(args, "s:removeBoneOverride", &name))
        Py_RETURN_NONE;

    arm->RemoveBoneOverride(name);
    Py_RETURN_NONE;
}


PyObject *BL_ArmatureObject::py_clearBoneOverrides(PyObject *self, PyObject *args)
{
    BL_ArmatureObject *arm =
        static_cast<BL_ArmatureObject*>(((EXP_PyObjectPlus_Proxy*)self)->ref);
    arm->ClearBoneOverrides();
    Py_RETURN_NONE;
}
