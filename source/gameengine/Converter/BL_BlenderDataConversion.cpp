...
KX_Mesh *BL_ConvertMesh(Mesh *me, Object *blenderobj, KX_Scene *scene, BL_SceneConverter& converter)
{
	...
	// Read custom property from Blender Mesh to allow/deny auto grouping.
	// Property name: "bge_allow_auto_batch" (bool/int/float/double).
	bool allow_auto_batch = true;
	if (me && me->id.properties) {
		IDProperty *p = IDP_GetPropertyFromGroup(me->id.properties, "bge_allow_auto_batch");
		if (p) {
			if (p->type == IDP_INT) {
				allow_auto_batch = (IDP_Int(p) != 0);
			}
			else if (p->type == IDP_FLOAT) {
				allow_auto_batch = (IDP_Float(p) != 0.0f);
			}
#ifdef IDP_DOUBLE
			else if (p->type == IDP_DOUBLE) {
				allow_auto_batch = (IDP_Double(p) != 0.0);
			}
#endif
		}
	}
	meshobj->SetAllowAutoBatching(allow_auto_batch);

	// Read custom property "bge_force_instancing" (bool/int/float/double).
	bool force_instancing = false;
	if (me && me->id.properties) {
		IDProperty *p = IDP_GetPropertyFromGroup(me->id.properties, "bge_force_instancing");
		if (p) {
			if (p->type == IDP_INT) {
				force_instancing = (IDP_Int(p) != 0);
			}
			else if (p->type == IDP_FLOAT) {
				force_instancing = (IDP_Float(p) != 0.0f);
			}
#ifdef IDP_DOUBLE
			else if (p->type == IDP_DOUBLE) {
				force_instancing = (IDP_Double(p) != 0.0);
			}
#endif
		}
	}
	meshobj->SetForceInstancing(force_instancing);
	...
	for (unsigned short i = 0; i < totmat; ++i) {
		...
		if (force_instancing) {
			bucket->GetMaterial()->SetForceInstancing(true);
		}
...
#define BL_CONVERTBLENDEROBJECT_SINGLE                                 \
	...
	// Calculate and set the maximum activity culling distance for the scene.
	float maxCullingDistance = 0.0f;
	for (KX_GameObject *gameobj : sumolist) {
		const KX_GameObject::ActivityCullingInfo& info = gameobj->GetActivityCullingInfo();
		if (info.m_flags & KX_GameObject::ActivityCullingInfo::ACTIVITY_PHYSICS) {
			float r = std::sqrt(info.m_physicsRadius);
			if (r > maxCullingDistance)
				maxCullingDistance = r;
		}
		if (info.m_flags & KX_GameObject::ActivityCullingInfo::ACTIVITY_LOGIC) {
			float r = std::sqrt(info.m_logicRadius);
			if (r > maxCullingDistance)
				maxCullingDistance = r;
		}
	}
	if (maxCullingDistance > 0.0f) {
		kxscene->SetActivityCullingMaxDistance(maxCullingDistance);
	}
