...
void KX_ObjectActuator::HardStop()
{
    KX_GameObject *parent = static_cast<KX_GameObject *>(GetParent());
    if (!parent) return;

    // Se for character motion, zera caminhada e estado de pulo
    if (m_bitLocalFlag.CharacterMotion) {
        if (PHY_ICharacter *ch = parent->GetScene()->GetPhysicsEnvironment()->GetCharacterController(parent)) {
            ch->SetWalkDirection(mt::zero3);
        }
        m_jumping = false;
    }

    // Zera alvos e entradas do atuador
    m_force = mt::zero3;
    m_torque = mt::zero3;
    m_dloc = mt::zero3;
    m_drot = mt::zero3;
    m_linear_velocity  = mt::zero3;
    m_angular_velocity = mt::zero3;
    m_linear_damping_active  = false;
    m_angular_damping_active = false;
    ...
    // FLUSH no corpo físico: velocidade 0 (em mundo, para garantir)
    parent->SetLinearVelocity(mt::zero3, /*local*/ false);
    parent->SetAngularVelocity(mt::zero3, /*local*/ false);
}
