...
class KX_GameObject : public SCA_IObject, public mt::SimdClassAllocator
{
	Py_Header
public:
    KX_BlenderMaterial* GetFirstBlenderMaterial() const;
    bool GetRenderVisibility() const;
    bool HasShadowCasterMaterial() const;
    /// Invalidate the shadow caster material cache (e.g. after ReloadMaterials).
    void InvalidateShadowCache() { m_shadowCacheDirty = true; }
...
protected:
	...
	mutable bool						m_shadowCacheDirty;
	/// BatchGroup pointer used when the shadow cache was last computed — detects mid-frame batch changes.
	mutable RAS_BatchGroup*				m_shadowCacheBatchGroup;
	/// Dirty flag for bucket properties (passIndex, layer, color) — set when they change.
	bool								m_bucketPropsDirty;
	...
	mt::Vector<double, 3> m_globalPosition;
...
public:
	...
	std::atomic<bool>                   m_proceduralCancel;
	std::atomic<int>                    m_proceduralActiveCalls;

	// Cache para surface_nets_and_rebuild com skip_sdf_fill
	// Usa aligned_allocator<32> para garantir vetorização AVX2 nos loops SIMD
	...
	AlignedFloatVec						m_voxel_grad_grid;  // SOA: [gx0..gxN, gy0..gyN, gz0..gzN]
	AlignedFloatVec						m_voxel_sdf_cache;  // cache do SDF para dirty-region
	int									m_voxel_grad_nx = 0;
	int									m_voxel_grad_ny = 0;
	int									m_voxel_grad_nz = 0;

	// Cache dos parâmetros FIXOS por chunk (nunca mudam entre frames):
	//   [0] seed  [1] height_scale  [2] freq  [3] h_cache_scale
	//   [4] noise_scale_multiplier  [5] oct2_freq_mult  [6] oct2_weight  [7] oct2_height_scale
	//   [8] smooth_factor  [9] smooth_iterations
	// Parâmetros dinâmicos (bounds, pos, clip, skip_sdf, seam) NÃO são cacheados.
	static constexpr int kSnrCacheSize = 10;
	float								m_snr_static[kSnrCacheSize] = {};
	bool								m_snr_static_valid = false;

	// GPU-instanced grass instancer (owned, created by enableGrass())
	class KX_GrassInstancer *m_grassInstancer = nullptr;
#endif
...
#ifdef WITH_PYTHON
	static PyObject *sPyGetVertexShader(PyObject *self, PyObject *args);
	static PyObject *sPyGetFragmentShader(PyObject *self, PyObject *args);
	static PyObject *sPyGetShadowVertexShader(PyObject *self, PyObject *args);
	static PyObject *sPyGetShadowFragmentShader(PyObject *self, PyObject *args);
	PyObject *PyGetVertexShader();
	PyObject *PyGetFragmentShader();
	PyObject *PyGetShadowVertexShader();
	PyObject *PyGetShadowFragmentShader();
#endif
	...
	virtual bool UnlinkObject(SCA_IObject *clientobj) override;
	...
	const mt::Vector<double, 3>& GetGlobalPosition() const { return m_globalPosition; }
	void SetGlobalPosition(const mt::Vector<double, 3>& pos) { m_globalPosition = pos; }
	 ...
	 */
	inline void UpdateActivity(float distance)
	{
	...
	static PyObject *sPyAddLinearVelocity(PyObject *self, PyObject *args);
	...
	EXP_PYMETHOD_O(KX_GameObject,DestroyMesh);
	EXP_PYMETHOD_NOARGS(KX_GameObject,CancelProcedural);
	EXP_PYMETHOD_NOARGS(KX_GameObject,ResetProceduralCancel);
	EXP_PYMETHOD_NOARGS(KX_GameObject,GetProceduralActiveCalls);
	...
	EXP_PYMETHOD_DOC(KX_GameObject,setGrassTextureObject);
	EXP_PYMETHOD_DOC(KX_GameObject,setGrassLightMultipliers);
	EXP_PYMETHOD_DOC(KX_GameObject,sendMessage);
	EXP_PYMETHOD(KX_GameObject, ReinstancePhysicsMesh);
	EXP_PYMETHOD_NOARGS(KX_GameObject, UpdateTerrainNormals);
	EXP_PYMETHOD_O(KX_GameObject, ReplacePhysicsShape);
	EXP_PYMETHOD_DOC(KX_GameObject, addDebugProperty);

	EXP_PYMETHOD_DOC_VARARGS(KX_GameObject, ComputeVoxelDistance);
	EXP_PYMETHOD_DOC_VARARGS(KX_GameObject, ApplyRecipe);
	EXP_PYMETHOD_DOC_VARARGS(KX_GameObject, SurfaceNetsGenerate);
	...
	EXP_PYMETHOD_DOC_VARARGS(KX_GameObject, DisableGrass);
	...
	/* attributes */
	PyObject* PySetMipmapping(PyObject *args);
	PyObject* PyUpdateMipmappingFilter(PyObject *args);
	PyObject* PyGetVisibleTerrainVertices(PyObject *args);
	PyObject* PyFilterMipmapping(PyObject *args);
	PyObject* PyuseMultithread(PyObject *args);
	PyObject *PyGetBiomeData(PyObject *args);
