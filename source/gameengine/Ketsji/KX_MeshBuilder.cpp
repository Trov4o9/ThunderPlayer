...
static bool convertSequenceToIndices(PyObject *value, std::vector<unsigned int>& out, const char *errmsg)
{
	PyObject *seq = PySequence_Fast(value, errmsg);
	if (!seq) {
		return false;
	}
	...
	for (Py_ssize_t i = 0; i < size; ++i) {
		const unsigned long long val = PyLong_AsUnsignedLongLong(items[i]);
		if (PyErr_Occurred()) {
			Py_DECREF(seq);
		...
		if (val > std::numeric_limits<unsigned int>::max()) {
			Py_DECREF(seq);
			PyErr_Format(PyExc_OverflowError, "%s", errmsg);
			return false;
		}

		out[i] = static_cast<unsigned int>(val);
	}
...
bool KX_MeshBuilderSlot::Invalid() const
{
	...
	switch (m_array->GetPrimitiveType()) {
		case RAS_DisplayArray::TRIANGLES:
			count = 3;
			break;
		case RAS_DisplayArray::LINES:
			count = 2;
			break;
		case RAS_DisplayArray::PATCHES:
			return false;
	}
...
RAS_DisplayArray *KX_MeshBuilderSlot::ExtractDisplayArray()
{
	RAS_DisplayArray *ret = m_array;
	m_array = new RAS_DisplayArray(ret->GetPrimitiveType(), ret->GetFormat());
	return ret;
}
...
PyObject *KX_MeshBuilderSlot::PyAddVertex(PyObject *args, PyObject *kwds)
{
	...
	if (pyuvs) {
		PyObject *seq = PySequence_Fast(pyuvs, "addVertex(uvs): expected a sequence");
		if (!seq) {
			return nullptr;
		}

		const unsigned short size = min_ii(format.uvSize, PySequence_Fast_GET_SIZE(seq));
		PyObject **items = PySequence_Fast_ITEMS(seq);
		for (unsigned short i = 0; i < size; ++i) {
			if (!PyVecTo(items[i], uvs[i])) {
				Py_DECREF(seq);
				return nullptr;
			}
		}
	...
	if (pycolors) {
		PyObject *seq = PySequence_Fast(pycolors, "addVertex(colors): expected a sequence");
		if (!seq) {
			return nullptr;
		}

		const unsigned short size = min_ii(format.colorSize, PySequence_Fast_GET_SIZE(seq));
		PyObject **items = PySequence_Fast_ITEMS(seq);
		for (unsigned short i = 0; i < size; ++i) {
			mt::vec4 color;
			if (!PyVecTo(items[i], color)) {
				Py_DECREF(seq);
				return nullptr;
			}
...
PyObject *KX_MeshBuilderSlot::PyAddIndex(PyObject *value)
{
	...
	if (!convertSequenceToIndices(value, indices, "expected a sequence of positive integers")) {
		return nullptr;
	}
	if (indices.empty()) {
		Py_RETURN_NONE;
	...
	m_array->ReservePrimitiveIndices(indices.size());
	m_array->AddPrimitiveIndices(indices.data(), indices.size());

	if (isTriangle) {
		m_array->ReserveTriangleIndices(indices.size());
		m_array->AddTriangleIndices(indices.data(), indices.size());
	}
...
{
	(void)args;
	PyErr_SetString(PyExc_NotImplementedError, "slot.removeVertex() is not supported in this UPBGE branch");
	return nullptr;
}
...
{
	(void)args;
	PyErr_SetString(PyExc_NotImplementedError, "slot.removePrimitiveIndex() is not supported in this UPBGE branch");
	return nullptr;
}
...
{
	(void)args;
	PyErr_SetString(PyExc_NotImplementedError, "slot.removeTriangleIndex() is not supported in this UPBGE branch");
	return nullptr;
}
...
{
	PyObject *pymat;
	int primitive = RAS_DisplayArray::TRIANGLES;
	...
	if (!ELEM(primitive, RAS_DisplayArray::LINES, RAS_DisplayArray::TRIANGLES, RAS_DisplayArray::PATCHES)) {
...
PyObject *KX_MeshBuilder::PyFinish()
{
	...
	for (unsigned short i = 0, size = m_slots.GetCount(); i < size; ++i) {
		...
		mesh->AddMaterial(bucket, i, slot->ExtractDisplayArray());
	}
