...
bool KX_SteeringActuator::Update(double curtime)
{
	...
	// Handle target position potentially being in a different scene/coordinate system
	mt::vec3 targpos;
	if (m_target->GetScene() != obj->GetScene()) {
		targpos = m_target->NodeGetWorldPosition();
	} else {
		targpos = m_target->NodeGetWorldPosition();
	}
...
PyObject *KX_SteeringActuator::py_get_path_item(unsigned int index)
{
	...
	if (m_navmesh) {
		scene = m_navmesh->GetScene();
	}
	else if (m_target) {
		scene = m_target->GetScene();
	}
	else {
		KX_GameObject *obj = static_cast<KX_GameObject *>(GetParent());
		if (obj) {
