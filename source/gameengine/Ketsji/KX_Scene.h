...
extern bool g_useMultithreadCulling;
...
struct VisibilityCacheKey {
	int layer;
	std::uintptr_t frustumPtr;

	bool operator==(const VisibilityCacheKey& other) const {
		return layer == other.layer && frustumPtr == other.frustumPtr;
	}
};

namespace std {
	template <>
	struct hash<VisibilityCacheKey> {
		std::size_t operator()(const VisibilityCacheKey& k) const {
			return std::hash<int>()(k.layer) ^ (k.frustumPtr << 1);
		}
	};
}
...
{
	mutable int m_lastFrame = -1;
	mutable std::int64_t m_cullingBudgetSecond = -1;
	mutable std::uint64_t m_cullingBudgetLastFrameCounter = 0;
	mutable int m_cullingBudgetFramesThisSecond = 0;
	mutable bool m_cullingBudgetRelaxedThisSecond = false;
	...
	mutable std::unordered_map<VisibilityCacheKey, VisibilityCacheEntry> m_visibilityCache;
...
private:
	...
	struct CullingInfo
	{
		...
		CullingInfo(int layer, std::vector<KX_GameObject *>& objects, bool update_state = true)
			:m_layer(layer),
			m_objects(objects),
	...
	std::unordered_set<RAS_BatchGroup *> m_shadowVisitedGroups;
	...
	/// Objects that have a LOD manager — subset of m_objectlist, avoids full scan in UpdateObjectLods.
	std::vector<KX_GameObject *> m_lodObjects;
	...
	/// List of objects with activity culling enabled for faster update.
	std::vector<KX_GameObject *> m_activityCullingList;
	/// Index for quantized activity culling updates.
	size_t m_activityCullingIndex;
	/// Cached camera positions for activity culling to avoid allocations.
	std::vector<mt::vec3, mt::simd_allocator<mt::vec3>> m_activityCullingCamPositions;
	/// Cached camera cell positions for activity culling spatial hashing.
	std::vector<std::pair<int, int>> m_activityCullingCamCells;

	/// Vetor reutilizado por frame para objetos visíveis — evita alocação de heap por frame.
	std::vector<KX_GameObject *> m_visibleObjects;
	/// Objetos dinâmicos (AutoUpdateBounds=true) — só eles precisam de reset de culling por frame.
	std::vector<KX_GameObject *> m_dynamicObjects;
	...
	/**
	 * Cache of group members for fast DupliGroup instantiation.
	 * Keyed by Blender Group pointer to avoid recomputation across spawns.
	 */
	std::unordered_map<void*, std::set<KX_GameObject *>> m_groupCache;
	...
	/// Max cell distance for activity culling (dynamic based on objects radius).
	int m_activityCullingMaxCellDist;
	float m_activityCullingMaxCellDistSq;
	...
	// C++ grass system — singleton por cena, drawn after main geometry pass
	class KX_GrassSystem *m_grassSystem = nullptr;

	// C++ terrain system — workers sem GIL para geração de chunks
	class KX_TerrainSystem *m_terrainSystem = nullptr;
...
public:
	...
	void RenderSolidBuckets(const std::vector<KX_GameObject *>& objects, RAS_Rasterizer::DrawType drawingMode,
	                        const mt::mat3x4& cameratransform, RAS_Rasterizer *rasty);
	/// Renderiza só alphas — chamado após a grama.
	void RenderAlphaBuckets(RAS_Rasterizer::DrawType drawingMode,
	                        const mt::mat3x4& cameratransform, RAS_Rasterizer *rasty, RAS_OffScreen *offScreen);
	...
	class KX_GrassSystem *GetGrassSystem() const { return m_grassSystem; }
	...
	std::vector<KX_GameObject *> CalculateVisibleMeshes(const SG_Frustum& frustum, int layer, bool update_state = true);
	void UpdateDbvt(bool update_state = true, bool update_bounds = true);
	...
	/// Register/unregister an object in the LOD-capable list.
	void AddLodObject(KX_GameObject *gameobj);
	void RemoveLodObject(KX_GameObject *gameobj);
	/// Register/unregister an object in the dynamic (auto-update-bounds) list.
	void AddDynamicObject(KX_GameObject *gameobj);
	void RemoveDynamicObject(KX_GameObject *gameobj);
	/// Invalidate the shadow caster material cache on all objects (call after ReloadMaterials).
	...
	/// Set max activity culling distance in units.
	void SetActivityCullingMaxDistance(float distance);
	...
	// --- Grass system (C++ only, no Python) ---
	KX_GrassSystem *GetOrCreateGrassSystem();
	void DrawGrassSystem(RAS_Rasterizer *rasty);
