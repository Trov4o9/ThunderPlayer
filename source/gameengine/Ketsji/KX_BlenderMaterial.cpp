...
void KX_BlenderMaterial::GetMaterialData(float& r, float& g, float& b, float& a,
                                         float& specr, float& specg, float& specb, float& spec,
                                         float& roughness, float& metallic)
{
    if (!m_material) {
        r = g = b = specr = specg = specb = spec = roughness = metallic = 0.0f;
        a = 1.0f;
        return;
    }
    ...
    specr = m_material->specr;
    specg = m_material->specg;
    specb = m_material->specb;
    spec = m_material->spec;

    roughness = m_material->roughness_bsdf;
    metallic = m_material->metallic_bsdf;
}
...
{
	if (!m_shader || !m_shader->Ok()) return;

	BL_Shader *blShader = static_cast<BL_Shader *>(m_shader);
	GLuint prog = (GLuint)m_shader->GetProg();

	for (unsigned short i = 0; i < RAS_Texture::MaxUnits; i++) {
		if (!m_textures[i] || !m_textures[i]->Ok()) continue;

		BL_Texture *tex = static_cast<BL_Texture *>(m_textures[i]);
		tex->ActivateTexture(i);  // ensures residency, no glActiveTexture/glBindTexture

		const unsigned long long handle = tex->GetBindlessHandle();
		const int loc = blShader->GetSamplerSlotLoc(i);
...
void KX_BlenderMaterial::Desactivate(RAS_Rasterizer *rasty)
{
	if (m_shader && m_shader->Ok()) {
		m_shader->UnbindProg();
		// Bindless: no per-slot disable needed — DisableTexture is a no-op.
	}
	else if (m_blenderShader && m_blenderShader->Ok()) {
...
void KX_BlenderMaterial::SetForceInstancing(bool force)
{
	if (m_forceInstancing == force)
		return;

	m_forceInstancing = force;
	ReloadMaterial();
}

bool KX_BlenderMaterial::GetForceInstancing() const
{
	return m_forceInstancing;
}
...
void KX_BlenderMaterial::ActivateMeshUser(RAS_MeshUser *meshUser, RAS_Rasterizer *rasty, const mt::mat3x4& camtrans, bool forceUpdate)
{
	const bool needsUpdate = forceUpdate || rasty->NeedsMeshUserUpdate(this, meshUser->GetMatrix(), meshUser->GetColor(),
	                                                    meshUser->GetLayer(), meshUser->GetRandom(),
	...
	if (needsUpdate) {
		rasty->UpdateMeshUserState(this, meshUser->GetMatrix(), meshUser->GetColor(),
		                           meshUser->GetLayer(), meshUser->GetRandom(),
		                           meshUser->GetPassIndex());
	}
}
...
PyAttributeDef KX_BlenderMaterial::Attributes[] = {
	...
	EXP_PYATTRIBUTE_RW_FUNCTION("forceInstancing", KX_BlenderMaterial, pyattr_get_forceInstancing, pyattr_set_forceInstancing),
...
PyObject *KX_BlenderMaterial::pyattr_get_forceInstancing(EXP_PyObjectPlus *self_v, const EXP_PYATTRIBUTE_DEF *attrdef)
{
	KX_BlenderMaterial *self = static_cast<KX_BlenderMaterial *>(self_v);
	return PyBool_FromLong(self->GetForceInstancing() ? 1 : 0);
}

int KX_BlenderMaterial::pyattr_set_forceInstancing(EXP_PyObjectPlus *self_v, const EXP_PYATTRIBUTE_DEF *attrdef, PyObject *value)
{
	KX_BlenderMaterial *self = static_cast<KX_BlenderMaterial *>(self_v);
	const int val = PyObject_IsTrue(value);
	if (val == -1) {
		PyErr_Format(PyExc_AttributeError, "material.%s = bool: KX_BlenderMaterial, expected a bool", attrdef->m_name.c_str());
		return PY_SET_ATTR_FAIL;
	}

	self->SetForceInstancing(val != 0);
