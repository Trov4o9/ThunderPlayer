	...
	:SCA_IActuator(gameobj, KX_ACT_SOUND)
{
	...
	m_type = type;
	m_isplaying = false;
	m_resumeAfterPause = false;
}

void KX_SoundActuator::StopSound()
{
...
bool KX_SoundActuator::IsPlaying() const
{
#ifdef WITH_AUDASPACE
    if (m_handle) {
        return AUD_Handle_getStatus(m_handle) == AUD_STATUS_PLAYING;
    }
#endif
    return false;
}
	...
	if (isplaying) {
		if (m_is3d) {
			...
			if (cam) {
				...
				// Calculate relative position using global coordinates to handle cross-scene cases and floating origin
				mt::vec3 camPosLocal = cam->NodeGetWorldPosition();
				mt::vec3 objPosLocal = obj->NodeGetWorldPosition();
				
				mt::vec3 diffGlobal = objPosLocal - camPosLocal;
				p[0] = (float)diffGlobal[0];
				p[1] = (float)diffGlobal[1];
				p[2] = (float)diffGlobal[2];
