...
static PyObject *gPySetFrustumCamera(PyObject *, PyObject *value)
{
	KX_Scene *scene = KX_GetActiveScene();
	if (!scene) {
		Py_RETURN_NONE;
	...
	if (!ConvertPythonToCamera(scene, value, &cam, true, "bge.logic.setFrustumCamera(camera): ")) {
		return nullptr;
	}

	scene->SetOverrideCullingCamera(cam);
	Py_RETURN_NONE;
}

PyDoc_STRVAR(gPyGetFrustumCamera_doc,
             "getFrustumCamera()\n"
             "Retorna a camera de override usada para o frustum de culling (ou None).\n");
static PyObject *gPyGetFrustumCamera(PyObject *)
{
	KX_Scene *scene = KX_GetActiveScene();
	if (!scene) {
		Py_RETURN_NONE;
	...
	KX_Camera *cam = scene->GetOverrideCullingCamera();
	if (cam) {
		return cam->GetProxy();
	}
	Py_RETURN_NONE;
...
// ============================================================================
// bge.logic.setGrassParams — configura parâmetros globais do sistema de grama
// Init é automático no primeiro Draw após enableGrass()
// ============================================================================
#include "KX_GrassSystem.h"

static PyObject *gPySetGrassParams(PyObject *, PyObject *args)
{
	float scale       = 1.0f;
	...
	float windLod     = -1.0f;  // -1 = automático baseado em distance
	float lightLod    = -1.0f;  // -1 = automático baseado em distance
	float lodStart    = -1.0f;  // -1 = automático (50% de distance)

	if (!PyArg_ParseTuple(args, "|ffffffffifffff",
	                      &scale, &lengthMax, &lengthMin, &heightScale,
	                      &distance, &strength, &speed, &range, &density, &flatness,
	                      &windLod, &lightLod, &lodStart))
		return nullptr;
	...
	GrassParams p;
	p.scale       = scale;
	p.lengthMax   = lengthMax;
	p.lengthMin   = lengthMin;
	p.heightScale = heightScale;
	p.distance    = distance;
	p.strength    = strength;
	p.speed       = speed;
	p.range       = range;
	p.density     = density; // negativo = densidade reduzida (1/|density| da base)
	p.flatness    = std::max(0.0f, std::min(1.0f, flatness));
	p.windLod     = windLod;
	p.lightLod    = lightLod;
	p.lodStart    = lodStart;

	scene->GetOrCreateGrassSystem()->SetParams(p);
	Py_RETURN_NONE;
}
...
static struct PyMethodDef game_methods[] = {
	...
	{"setFrustumCamera", (PyCFunction)gPySetFrustumCamera, METH_O, (const char *)"set Frustum Camera"},
	{"getFrustumCamera", (PyCFunction)gPyGetFrustumCamera, METH_NOARGS, (const char *)gPyGetFrustumCamera_doc},
	{"setGrassParams", (PyCFunction)gPySetGrassParams, METH_VARARGS,
	 (const char *)"setGrassParams(scale, lengthMax, lengthMin, heightScale, distance, strength, speed, range, density, flatness, windLod=-1, lightLod=-1) — configure global grass system. windLod/lightLod: distância explícita de LOD para vento/luz; -1 = automático baseado em distance"},
		...
		if (KX_GetActiveEngine()) {
			...
			for (KX_Scene *scene : scenes) {
				...
				if (scene->GetBucketManager()) {
					...
					scene->InvalidateAllShadowCaches();
				}
			}
		}
	}
...
static PyObject *gPySetRenderFpsLimit(PyObject *, PyObject *args)
{
    double limit;

    if (!PyArg_ParseTuple(args, "d:setRenderFpsLimit", &limit)) {
        return nullptr;
    }

    if (limit < 0.0) {
        PyErr_SetString(PyExc_ValueError, "Rasterizer.setRenderFpsLimit(limit): Expected a non-negative value");
        return nullptr;
    }

    g_render_fps_limit = limit;

    Py_RETURN_NONE;
}

#include "Deferred.h"
bool g_useDeferred_G = false;

static PyObject *gPySetUseDeferred(PyObject *, PyObject *args)
{
    PyObject *value;

    if (!PyArg_ParseTuple(args, "O:setUseDeferred", &value)) {
        return nullptr;
    }

    if (!PyBool_Check(value)) {
        PyErr_SetString(PyExc_TypeError, "Rasterizer.setUseDeferred(flag): Expected a boolean value (True or False)");
        return nullptr;
    }

    //g_useDeferred = (value == Py_True);
	//g_useDeferred_G = g_useDeferred;
...
static struct PyMethodDef rasterizer_methods[] = {
	...
	{"SetRenderFpsLimit", (PyCFunction)gPySetRenderFpsLimit,
	 METH_VARARGS, "set fps render limit"},
	{"SetUseDeferred", (PyCFunction)gPySetUseDeferred,
	 METH_VARARGS, "set use deferred"},
