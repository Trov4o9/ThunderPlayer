...
bool KX_MouseFocusSensor::ParentObjectHasFocusCamera(KX_Camera *cam)
{
	...
	const RAS_Rect displayArea = rasty->GetRenderAreaMouse(canvas, rasty->GetStereoMode(), RAS_Rasterizer::RAS_STEREO_LEFTEYE);
	m_kxengine->GetSceneViewport(m_kxscene, cam, displayArea, area, viewport);

	//cam->UpdateView(rasty, m_kxscene, rasty->GetStereoMode(), RAS_Rasterizer::RAS_STEREO_LEFTEYE, viewport, area);
