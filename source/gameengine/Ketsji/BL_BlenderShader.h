...
private:
	...
	// Cache da view matrix para evitar UpdateLights redundante quando a câmera não se move
	float m_cachedViewMat[4][4];
...
public:
	...
	void SetCustomGLSL(const char *code);
	std::string m_customGLSL;
	...
	void ReloadMaterial();
	int GetAlphaBlend();
	struct GPUMaterial *GetGPUMaterial() { return m_gpuMat; }
};
