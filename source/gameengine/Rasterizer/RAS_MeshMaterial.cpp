	  ...
	  m_mesh(mesh)
{
	...
	if (m_mesh && m_mesh->GetName().find("Tess") != std::string::npos) {
		printf("[RAS_MeshMaterial] Tessellation detectado no mesh: %s\n", m_mesh->GetName().c_str());
		type = RAS_DisplayArray::PATCHES;
	}
	else {
		type = (bucket->IsWire()) ? RAS_DisplayArray::LINES : RAS_DisplayArray::TRIANGLES;
	}
