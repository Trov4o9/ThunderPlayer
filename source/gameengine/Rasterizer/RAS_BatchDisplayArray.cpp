...
unsigned int RAS_BatchDisplayArray::Merge(RAS_DisplayArray *array, const mt::mat4& mat)
{
	...
	const auto& srcPos = array->m_vertexData.positions;
	const auto& srcNor = array->m_vertexData.normals;
	const auto& srcTan = array->m_vertexData.tangents;
	mt::vec3_packed* dstPos = m_vertexData.positions.data() + startvertex;
	mt::vec3_packed* dstNor = m_vertexData.normals.data() + startvertex;
	mt::vec4_packed* dstTan = m_vertexData.tangents.data() + startvertex;
	for (unsigned int i = 0; i < vertexcount; ++i) {
		const mt::vec3 p(srcPos[i]);
		const mt::vec3 n(srcNor[i]);
		const mt::vec4 t(srcTan[i]);
		dstPos[i] = (mat * p);
		dstNor[i] = (nmat * n);
		dstTan[i] = (nmat * t);
	}
	...
	for (unsigned short i = 0; i < m_format.uvSize; ++i) {
		m_vertexData.uvs[i].reserve(m_vertexData.uvs[i].size() + vertexcount);
	...
	for (unsigned short i = 0; i < m_format.colorSize; ++i) {
		m_vertexData.colors[i].reserve(m_vertexData.colors[i].size() + vertexcount);
		m_vertexData.colors[i].insert(m_vertexData.colors[i].end(), array->m_vertexData.colors[i].begin(), array->m_vertexData.colors[i].end());
	}

	unsigned int* dstIdx = m_primitiveIndices.data() + startindex;
	const unsigned int* srcIdx = array->m_primitiveIndices.data();
	for (unsigned int i = 0; i < indexcount; ++i) {
		dstIdx[i] = srcIdx[i] + startvertex;
	}
