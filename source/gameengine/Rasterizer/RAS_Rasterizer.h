...
private:
	...
	inline void PrepareDrawOffScreen(RAS_ICanvas *canvas);
	inline void DrawOffScreenWithShader(RAS_OffScreen *src, GPUShader *shader, int tex0 = 0, int tex1 = -1, bool bindTex0 = true, bool bindTex1 = false);
	void *m_clientobject;
	...
	/// Current GPUMaterial being rendered (for custom shadow shaders)
	struct GPUMaterial *m_currentGPUMaterial;
	...
	struct MeshUserState {
		...
		MeshUserState() : material(nullptr), layer(0), random(0.0f), passIndex(0) {}
	} m_lastMeshUserState;
...
public:
	...
	bool NeedsMeshUserUpdate(void *material, const mt::mat4& matrix, const mt::vec4& color,
	                         unsigned int layer, float random, short passIndex);

	void UpdateMeshUserState(void *material, const mt::mat4& matrix, const mt::vec4& color,
	                         unsigned int layer, float random, short passIndex);
	...
	RAS_Rect GetRenderAreaMouse(RAS_ICanvas *canvas, StereoMode stereoMode, StereoEye eye);
	...
	/// Set current GPUMaterial being rendered (for custom shadow shaders)
	void SetCurrentGPUMaterial(struct GPUMaterial *material);
	
	/// Get current GPUMaterial being rendered
	struct GPUMaterial *GetCurrentGPUMaterial() const;
	...
	void BindOverrideShaderUniforms();  // Bind TIME and other custom uniforms
