...
RAS_2DFilter::RAS_2DFilter(RAS_2DFilterData& data)
	...
	m_filterMode(static_cast<RAS_2DFilterManager::FILTER_MODE>(data.filterMode))
{
...
RAS_OffScreen *RAS_2DFilter::Render(RAS_Rasterizer *rasty, RAS_ICanvas *canvas, 
                                    RAS_OffScreen *depthofs,
                                    ...
                                    RAS_OffScreen *targetofs)
{
    ...
    if (m_offScreen) {
        const int neww = canvas->GetWidth();
        const int newh = canvas->GetHeight();

        if (m_offScreen->GetWidth() != neww || m_offScreen->GetHeight() != newh) {
...
static bool g_smaaTexturesLoaded = false;
static GLuint g_smaaAreaTex = 0;
static GLuint g_smaaSearchTex = 0;

void RAS_2DFilter::BindTextures(RAS_OffScreen *depthofs, RAS_OffScreen *colorofs)
{
    if (!g_smaaTexturesLoaded)
    {
        g_smaaTexturesLoaded = true;

        // AreaTex (R8G8)
        glGenTextures(1, &g_smaaAreaTex);
        glBindTexture(GL_TEXTURE_2D, g_smaaAreaTex);
    ...
    g_DebugDepthTex = depthofs->GetDepthTexture();
...
void RAS_2DFilter::UnbindTextures(RAS_OffScreen *depthofs, RAS_OffScreen *colorofs)
{
    if (m_predefinedUniforms[RENDERED_TEXTURE_UNIFORM] != -1)
    {
        ...
        if (m_mipmap) colorofs->UnmipmapTextures();
    }
...
void RAS_2DFilter::BindUniforms(RAS_ICanvas *canvas)
{
	...
	if (m_predefinedUniforms[TEXTURE_COORDINATE_OFFSETS_UNIFORM] != -1) {
		...
		SetUniformfv(m_predefinedUniforms[TEXTURE_COORDINATE_OFFSETS_UNIFORM], RAS_Uniform::UNI_FLOAT2, m_textureOffsets.data(),
		             sizeof(float) * TEXTURE_OFFSETS_SIZE, TEXTURE_OFFSETS_SIZE / 2);
	}

    if (m_predefinedUniforms[SMAA_AREA_TEX_UNIFORM] != -1)
        SetUniform(m_predefinedUniforms[SMAA_AREA_TEX_UNIFORM], 6);

    if (m_predefinedUniforms[SMAA_SEARCH_TEX_UNIFORM] != -1)
        SetUniform(m_predefinedUniforms[SMAA_SEARCH_TEX_UNIFORM], 7);
