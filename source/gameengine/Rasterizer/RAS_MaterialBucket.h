...
{
public:
	RAS_MaterialBucket(RAS_IMaterial *mat, RAS_BucketManager* manager);
	...
	bool IsActive() const;
	void MarkActive();
	void MarkDisplayArrayBucketActive(RAS_DisplayArrayBucket *bucket);
	...
	// Batch group per-frame cache delegated to manager.
	RAS_BatchGroup* GetOrCreateFrameBatchGroup(size_t key, RAS_MeshUser* refUser);
	static size_t ComputeBatchKey(RAS_Mesh *mesh, RAS_DisplayArray *array, RAS_IMaterial *mat);

private:
	RAS_BucketManager* m_manager;
	RAS_IMaterial *m_material;
	RAS_DisplayArrayBucketList m_displayArrayBucketList;
	RAS_DisplayArrayBucketList m_activeDisplayArrayBucketList;
	size_t m_lastActiveDisplayArrayBucketCount;
	bool m_isActive;
