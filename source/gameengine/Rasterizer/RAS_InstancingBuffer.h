...
private:
	...
	/// The OpenGL VBO.
	unsigned int m_vbo;
	/// Mapeamento persistente do VBO (quando suportado).
	std::uint8_t *m_persistentPtr;
	/// Tamanho em bytes de um segmento (1 frame) do buffer.
	std::size_t m_segmentSize;
	/// Quantidade de segmentos no ring buffer (persistent mapping).
	unsigned int m_ringSize;
	/// Próximo índice no ring buffer para escrita.
	unsigned int m_nextRingIndex;
	/// Índice do segmento atualmente escrito (para fence após draw).
	unsigned int m_writeRingIndex;
	/// Fence por segmento para evitar sobrescrever dados ainda em uso pelo GPU.
	void *m_fences[3];
	/// Maximum number of instances currently allocated in VBO.
	unsigned int m_capacity;
	/// Offset base do segmento atual dentro do VBO (em bytes).
...
public:
	...
	/// Deve ser chamado após o draw que consome o último Update() (apenas no caminho persistent mapping).
	void SubmitFence();

	inline intptr_t GetMatrixOffset() const
	{
		return m_baseOffset + m_matrixOffset;
	}
	inline intptr_t GetPositionOffset() const
	{
		return m_baseOffset + m_positionOffset;
	}
	inline intptr_t GetColorOffset() const
	{
		return m_baseOffset + m_colorOffset;
	}

	inline intptr_t GetLayerOffset() const
	{
		return m_baseOffset + m_layerOffset;
	}

	inline intptr_t GetInfoOffset() const
	{
		return m_baseOffset + m_infoOffset;
	}
};
