...
private:
	...
	public:
		...
		ankerl::unordered_dense::map<RAS_MeshSlot *, RAS_DisplayArrayBucket *> m_originalDisplayArrayBucketList;
		/// All the mesh slots sorted by batch index.
		RAS_MeshSlotList m_meshSlots;
	};

	/// The batch per material.
	ankerl::unordered_dense::map<RAS_IMaterial *, Batch> m_batchs;

	/// All the mesh users using this batch group.
	std::vector<RAS_MeshUser *> m_meshUsers;

	/// Indices of slots that cast shadows, shared by all users of this group.
	std::vector<int> m_shadowSlotsIndices;
...
public:
	...
	/// Reseta o estado para reutilização pelo pool — não destrói o objeto.
	void Reset();

	short GetUsers() const { return m_users; }
	...
	const std::vector<int>& GetShadowSlotsIndices() const { return m_shadowSlotsIndices; }
	...
	/// Activate shadow mesh slots for all users in this batch group.
	void ActivateShadowForGroup();
};
