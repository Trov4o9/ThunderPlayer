...
unsigned int RAS_DisplayArray::AddVertex(
		const mt::vec3_packed& pos, const mt::vec3_packed& nor, const mt::vec4_packed& tan,
		mt::vec2_packed uvs[RAS_Texture::MaxUnits], unsigned int colors[RAS_Texture::MaxUnits],
		unsigned int origIndex, uint8_t flag)
{
...
{
    m_vertexData.positions.resize(0);
    m_vertexData.normals.resize(0);
    m_vertexData.tangents.resize(0);

    for (unsigned short i = 0; i < m_format.uvSize; ++i)
        m_vertexData.uvs[i].resize(0);

    for (unsigned short i = 0; i < m_format.colorSize; ++i)
        m_vertexData.colors[i].resize(0);
...
void RAS_DisplayArray::SortPolygons(const mt::mat3x4& transform, unsigned int *indexmap)
{
    ...
    if (m_polygonCenters.size() != totpoly) {
        m_polygonCenters.assign(totpoly, mt::zero3);
    ...
    for (unsigned int i = 0; i < totpoly; ++i) {
        ...
        indexmap[i * 3 + 0] = m_primitiveIndices[first + 0];
        indexmap[i * 3 + 1] = m_primitiveIndices[first + 1];
        indexmap[i * 3 + 2] = m_primitiveIndices[first + 2];
    }
}
...
RAS_DisplayArrayLayout RAS_DisplayArray::GetLayout() const
{
	...
	layout.rawColors = m_format.rawColors;
