...
public:
	...
	virtual void SyncMotionStates(float timeStep);
	virtual void SyncMotionStatesInterpolated(float timeStep, float alpha);
	virtual void CachePreviousTransforms();
...
//		virtual bool		proceedDeltaTimeOneStep(float timeStep);
	...
	bool CullingTestKX(std::vector<KX_GameObject *>& objects, int layer, bool updateState, const std::array<mt::vec4, 6>& planes,
	                   int planeCount,
	                   int occlusionRes, const int *viewport, const mt::mat4& matrix);

	void BeginOcclusionBuffer(int occlusionRes, const int *viewport, const mt::mat4& matrix);
	void SubmitOccluder(KX_GameObject *gameobj);
	bool QueryOcclusionAabb(const mt::vec3& aabbMin, const mt::vec3& aabbMax);
	void RegisterStaticOccluder(KX_GameObject *gameobj);
	void UnregisterStaticOccluder(KX_GameObject *gameobj);
...
protected:
	std::unordered_set<CcdPhysicsController *> m_controllers;

	PHY_ResponseCallback m_triggerCallbacks[PHY_NUM_RESPONSE];
	void *m_triggerCallbacksUserPtrs[PHY_NUM_RESPONSE];

#ifdef WITH_PYTHON
	struct ReinstanceAsyncState;
	ReinstanceAsyncState *m_reinstanceAsync = nullptr;
	std::atomic<uint64_t> m_reinstanceGeneration {1};
	void StartReinstanceAsyncIfNeeded();
	void ShutdownReinstanceAsync();
	void ProcessFinishedReinstanceJobs();
#endif

	std::vector<WrapperVehicle *>    m_wrapperVehicles;
	struct StaticOccluderCache {
		KX_GameObject *object = nullptr;
		float aabbMin[3] = {};
		float aabbMax[3] = {};
		std::vector<float> triangles;
		std::vector<float> lodTriangles;
		int gpuFirstVertex = -1;
		int gpuVertexCount = 0;
	};
	std::vector<StaticOccluderCache> m_staticOccluders;
	ankerl::unordered_dense::map<KX_GameObject *, size_t> m_staticOccluderIndex;
	btDbvt *m_staticOccluderDbvt = nullptr;
	ankerl::unordered_dense::map<KX_GameObject *, btDbvtNode *> m_staticOccluderDbvtNodes;
	uint32_t m_staticOccludersRevision = 1;
	bool m_occlusionGpuResetDone = false; // Reset do ring buffer GPU na 1ª sessão
