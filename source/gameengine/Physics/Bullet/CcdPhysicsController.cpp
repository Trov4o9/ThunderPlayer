	...
	:m_cci(ci)
{
	...
	m_savedDyna = false;
	m_suspended = false;
	m_previousTransform = CcdPhysicsController::GetTransformFromMotionState(m_MotionState);
...
bool CcdPhysicsController::SynchronizeMotionStatesInterpolated(float time, float alpha)
{
	btRigidBody *body = GetRigidBody();

	if (body && !body->isStaticObject()) {
		const btTransform& currentTransform = body->getCenterOfMassTransform();
		btTransform interpolatedTransform;

		interpolatedTransform.setOrigin(m_previousTransform.getOrigin().lerp(currentTransform.getOrigin(), alpha));

		btQuaternion rotA = m_previousTransform.getRotation();
		btQuaternion rotB = currentTransform.getRotation();

		// Use shortest path for SLERP to avoid 360-degree flips during interpolation
		if (rotA.dot(rotB) < 0.0f) {
			rotB = -rotB;
		}
		interpolatedTransform.setRotation(rotA.slerp(rotB, alpha));

		m_MotionState->SetWorldOrientation(ToMt(interpolatedTransform.getBasis()));
		m_MotionState->SetWorldPosition(ToMt(interpolatedTransform.getOrigin()));
		m_MotionState->CalculateWorldTransformations();
	}
...
void CcdPhysicsController::CachePreviousTransform()
{
	btRigidBody *body = GetRigidBody();
	if (body) {
		m_previousTransform = body->getCenterOfMassTransform();
	}
	else {
		m_previousTransform = CcdPhysicsController::GetTransformFromMotionState(m_MotionState);
	}
...
void CcdPhysicsController::RelativeTranslate(const mt::vec3& dlocin, bool local)
{
	if (m_object) {
		...
		if (local) {
			...
			if (basis[0][0] == 1.0f && basis[1][1] == 1.0f && basis[2][2] == 1.0f &&
				basis[0][1] == 0.0f && basis[0][2] == 0.0f &&
				basis[1][0] == 0.0f && basis[1][2] == 0.0f &&
				basis[2][0] == 0.0f && basis[2][1] == 0.0f) {
				// skip
			}
			else {
				dloc = basis * dloc;
...
bool CcdShapeConstructionInfo::ApplyAsyncMesh(btTriangleIndexVertexArray *meshInterface, std::vector<int>&& polygonIndexArray, std::vector<UVco>&& triFaceUVcoArray, RAS_Mesh *meshobj, RAS_Deformer *deformer, btTriangleIndexVertexArray **r_oldMeshInterface)
{
	if (!meshInterface || !meshobj) {
		return false;
	}
	...
	btTriangleIndexVertexArray *oldMeshInterface = m_triangleIndexVertexArray;
	m_triangleIndexVertexArray = meshInterface;
	m_polygonIndexArray = std::move(polygonIndexArray);
	m_triFaceUVcoArray = std::move(triFaceUVcoArray);
	m_forceReInstance = false;

	m_meshShapeMap[MeshShapeKey(meshobj, deformer, m_shapeType)] = this;
	m_mesh = meshobj;

	if (r_oldMeshInterface) {
		*r_oldMeshInterface = oldMeshInterface;
	}
	else if (oldMeshInterface) {
		delete oldMeshInterface;
	}
