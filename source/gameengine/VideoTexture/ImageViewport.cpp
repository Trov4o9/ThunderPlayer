...
void ImageViewport::calcViewport(unsigned int texId, double ts, bool mipmap, unsigned int format)
{
	...
	if (!m_texInit && texId != 0) {
		// initialize it
		const int levels = mipmapLevelsForTexture(m_size, mipmap);
		ensureTextureStorage(texId, m_size, levels, m_internalFormat);
		m_texInit = true;
	}
	...
	if (texId != 0 && m_pyfilter == nullptr && m_size[0] == m_capSize[0] &&
		...
		glCopyTextureSubImage2D(texId, 0, 0, 0, m_upLeft[0], m_upLeft[1], (GLsizei)m_capSize[0], (GLsizei)m_capSize[1]);
		if (mipmap) {
