...
void RNA_def_material(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Custom Vertex Shader", "Text data-block containing custom GLSL vertex shader code");
	RNA_def_property_update(prop, 0, "rna_Material_draw_update");

	/* Custom Fragment Shader */
	prop = RNA_def_property(srna, "custom_fragment_shader", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "custom_fragment_shader");
	RNA_def_property_struct_type(prop, "Text");
	RNA_def_property_flag(prop, PROP_EDITABLE | PROP_ID_REFCOUNT);
