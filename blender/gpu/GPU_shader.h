...
/* Shader creation */
GPUShader *GPU_shader_create_tess(
        const char *vertexcode,
        const char *fragcode,
        const char *geocode,
        const char *libcode,
...
GPUShader *GPU_shader_create_ex_tess(
        const char *vertexcode,
        const char *fragcode,
        const char *geocode,
        const char *libcode,
...
/* Shadow shader types for per-material shadow shaders */
typedef enum GPUShadowShaderType {
	GPU_SHADOW_SHADER_BLACK = 0,
	GPU_SHADOW_SHADER_BLACK_INSTANCING = 1,
	GPU_SHADOW_SHADER_VSM = 2,
	GPU_SHADOW_SHADER_VSM_INSTANCING = 3,
} GPUShadowShaderType;

GPUShader *GPU_shader_get_builtin_shader(GPUBuiltinShader shader);
GPUShader *GPU_shader_get_builtin_fx_shader(int effects, bool persp);

/* Get material-aware shadow shader (with custom vertex code injection if needed) */
struct GPUMaterial;
GPUShader *GPU_shader_get_material_shadow_shader(struct GPUMaterial *material, GPUShadowShaderType shadow_type);
