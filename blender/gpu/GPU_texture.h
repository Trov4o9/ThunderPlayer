...
/* Bindless texture API (GL_ARB_bindless_texture).
 * Uses unsigned long long (== GLuint64) to avoid requiring GL headers here. */
unsigned long long GPU_texture_bindless_handle(const GPUTexture *tex);
int GPU_texture_make_bindless_resident(GPUTexture *tex);   /* returns bool as int */
void GPU_texture_invalidate_bindless(GPUTexture *tex);

/* DSA-based filter update — works without binding, safe in bindless mode. */
