...
{
	rgba_char_args_set(ts->header_text,    255, 255, 255, 255);
	rgba_char_args_set(ts->header_title,   255, 255, 255, 255);
	rgba_char_args_set(ts->header_text_hi, 255, 255, 255, 255);

#if 0
	rgba_char_args_set(ts->panel_text,     255, 255, 255, 255);
	rgba_char_args_set(ts->panel_title,        255, 255, 255, 255);
	...
	rgba_char_args_set(ts->panelcolors.header, 0, 0, 155, 25);

	rgba_char_args_set(ts->button,         145, 145, 145, 245);
	rgba_char_args_set(ts->button_title,   255, 255, 255, 255);
	rgba_char_args_set(ts->button_text,    255, 255, 255, 255);
	rgba_char_args_set(ts->button_text_hi, 255, 255, 255, 255);

	rgba_char_args_set(ts->list,           165, 165, 165, 255);
	rgba_char_args_set(ts->list_title,     255, 255, 255, 255);
	rgba_char_args_set(ts->list_text,      255, 255, 255, 255);
...
void ui_theme_init_default(void)
{
	...
	rgba_char_args_set(btheme->tv3d.text,       200, 205, 210, 255);
	rgba_char_args_set(btheme->tv3d.text_hi, 255, 255, 255, 255);

	rgba_char_args_set_fl(btheme->tv3d.header,  0.0, 0.0, 0.155, 1.0);
	rgba_char_args_set_fl(btheme->tv3d.button,  0.65, 0.65, 0.65, 0.5);
//	rgba_char_args_set(btheme->tv3d.panel,      165, 165, 165, 127);
	...
	rgba_char_args_set(btheme->tv3d.header_text, 255, 255, 255, 255);
	rgba_char_args_set(btheme->tv3d.header_text_hi, 200, 210, 255, 255);
	rgba_char_args_set(btheme->tv3d.button_text, 255, 255, 255, 255);
	rgba_char_args_set(btheme->tv3d.button_text_hi, 200, 210, 255, 255);
	rgba_char_args_set(btheme->tv3d.button_title, 255, 255, 255, 255);
	rgba_char_args_set(btheme->tv3d.title, 255, 255, 255, 255);
	...
	rgba_char_args_set_fl(btheme->tbuts.back,   0.0, 0.0, 0.0, 1.0);
//	rgba_char_args_set(btheme->tbuts.panel, 0x82, 0x82, 0x82, 255);

	/* graph editor */
	btheme->tipo = btheme->tv3d;
	rgba_char_args_set_fl(btheme->tipo.back,    0.0, 0.0, 0.0, 1.0);
...
//	rgba_char_args_set(btheme->tfile.panel, 145, 145, 145, 255);  /* bookmark/ui regions */
	...
	rgba_char_args_set(btheme->tseq.back,   0, 0, 0, 255);
	...
	rgba_char_args_set(btheme->text.back,   0, 0, 0, 255);
	rgba_char_args_set(btheme->text.shade1,     143, 143, 143, 255);
	rgba_char_args_set(btheme->text.shade2,     0xc6, 0x14, 0x14, 255);
	rgba_char_args_set(btheme->text.hilite,     255, 0, 0, 255);

	/* syntax highlighting */
	rgba_char_args_set(btheme->text.syntaxn,    0, 0, 220, 255);    /* Numbers  Blue*/
	rgba_char_args_set(btheme->text.syntaxl,    255, 0, 0, 255);    /* Strings  Red */
	rgba_char_args_set(btheme->text.syntaxc,    0, 100, 50, 255);   /* Comments  Greenish */
	rgba_char_args_set(btheme->text.syntaxv,    0, 150, 220, 255);    /* Special  Yellow*/
	...
	rgba_char_args_set(btheme->text.syntaxb,    0, 150, 220, 255);   /* Builtin  Red-purple */
	rgba_char_args_set(btheme->text.syntaxs,    76, 76, 76, 255);   /* Gray (mix between fg/bg) */

	/* space oops */
	btheme->toops = btheme->tv3d;
	rgba_char_args_set_fl(btheme->toops.back,   0.0, 0.0, 0.0, 1.0);
	...
	rgba_char_args_set_fl(btheme->tuserpref.back, 0.0, 0.0, 0.0, 1.0);
	...
	rgba_char_args_set_fl(btheme->ttime.back,   0.0, 0.0, 0.0, 1.0);
	...
	rgba_char_args_set(btheme->tlogic.back, 0, 0, 0, 255);
	...
	if (!USER_VERSION_ATLEAST(237, 0)) {
		...
		for (btheme = U.themes.first; btheme; btheme = btheme->next) {
			...
			if (btheme->text.syntaxn[3] == 0) {
				...
				rgba_char_args_set(btheme->text.syntaxc,    0, 100, 50, 255);   /* Comments greenish */
				rgba_char_args_set(btheme->text.syntaxv,    0, 150, 200, 255);    /* Special */
				rgba_char_args_set(btheme->text.syntaxb,    0, 150, 220, 255);   /* Builtin, red-purple */
			}
		}
	}
...
/* TEMPORAL, remove me! (ton) */
	...
	if (!USER_VERSION_ATLEAST(260, 5)) {
		...
		for (btheme = U.themes.first; btheme; btheme = btheme->next) {
			rgba_char_args_set(btheme->tui.panel.header, 0, 0, 155, 25);
