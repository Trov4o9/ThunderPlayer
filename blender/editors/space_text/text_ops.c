...
#include "../../gpu/GPU_basic_shader.h"
...
static int text_compile_shader_exec(bContext *C, wmOperator *op)
{
    Text *text = CTX_data_edit_text(C);
    if (!text) {
        BKE_report(op->reports, RPT_ERROR, "Nenhum texto ativo para compilar");
        return OPERATOR_CANCELLED;
    }
    ...
    if (!f) {
        BKE_reportf(op->reports, RPT_ERROR, "Falha ao criar o arquivo: %s", filename);
        return OPERATOR_CANCELLED;
    }

    /* Escreve o conteúdo do Text no arquivo */
    for (TextLine *line = text->lines.first; line; line = line->next) {
        fprintf(f, "%s\n", line->line ? line->line : "");
    }
    ...
    BKE_reportf(op->reports, RPT_WARNING,
                "Shader salvo em '%s'. Reinicie o Blender.exe para aplicar.", filename);
...
void TEXT_OT_compile_shader(wmOperatorType *ot)
{
	/* identifiers */
	ot->name = "Compile Shader";
	ot->idname = "TEXT_OT_compile_shader";
	ot->description = "Export shader from text editor to gpu_shader_material.glsl";

	/* api callbacks */
	ot->exec = text_compile_shader_exec;
	ot->poll = text_edit_poll;
