...
KX_BlenderCanvas::KX_BlenderCanvas(RAS_Rasterizer *rasty, const RAS_OffScreen::AttachmentList& attachments,
		wmWindowManager *wm, wmWindow *win, RAS_Rect &rect, int winSizeX, int winSizeY, int numSamples)
...
void KX_BlenderCanvas::Resize(int width, int height)
{
	...
	// In embedded mode, width/height passed to Resize are usually the render resolution.
	// But we also need to keep track of the actual window size if it changes.
	// For now, let's assume m_winSizeX/Y are updated elsewhere or don't change often in embedded mode.
	// Actually, if we don't know the new window size, we can't update g_vpwi correctly here.
	// However, if Resize is called, it's likely that g_vpwi should be updated too.
