...
{
	SG_Node *node = *replica;

	if (!ActivateReplicationCallback(node)) {
		delete node;
		*replica = nullptr;
		return;
	}
	...
	if (!m_children.empty()) {
		node->ClearSGChildren();
		node->m_children.reserve(m_children.size()); 

		for (SG_Node *child : m_children) {
			if (SG_Node *replicanode = child->GetReplica()) {
				node->AddChild(replicanode);
			}
		}
	}

	if (node->m_children.empty() && !node->GetClientObject()) {
		delete node;
		*replica = nullptr;
	}
}
...
{
	bool updated = ComputeWorldTransforms(m_parent, parentUpdated);
	if (updated) {
	...
	for (SG_Node *childnode : m_children) {
		if (!parentUpdated && !childnode->IsModified()) {
			continue;
		}
...
void SG_Node::UpdateWorldDataThreadSchedule(bool parentUpdated)
{
    ...
    static bool poolInit = false;
    static ThreadPool* pool = nullptr;

    if (!poolInit) {
        unsigned int hwThreads = std::thread::hardware_concurrency();
        if (hwThreads == 0) hwThreads = 4; 
        pool = new ThreadPool(hwThreads);
        poolInit = true;
    }

	std::vector<SG_Node*> childrenToUpdate;
	childrenToUpdate.reserve(m_children.size());
	for (SG_Node* childnode : m_children) {
		if (!parentUpdated && !childnode->IsModified()) {
			continue;
		}
		childrenToUpdate.push_back(childnode);
	}

	if (childrenToUpdate.size() > 2) {
    	std::vector<std::future<void>> futures;
    	futures.reserve(childrenToUpdate.size());

    	for (SG_Node* childnode : childrenToUpdate) {
        	futures.push_back(pool->enqueue([childnode, parentUpdated]() {
...
{
	if (local) {
		if (m_localRotation(0, 0) == 1.0f && m_localRotation(1, 1) == 1.0f && m_localRotation(2, 2) == 1.0f &&
			m_localRotation(0, 1) == 0.0f && m_localRotation(0, 2) == 0.0f &&
			m_localRotation(1, 0) == 0.0f && m_localRotation(1, 2) == 0.0f &&
			m_localRotation(2, 0) == 0.0f && m_localRotation(2, 1) == 0.0f) {
		...
		if (parent) {
			const mt::mat3& worldOri = parent->GetWorldOrientation();
			if (worldOri(0, 0) == 1.0f && worldOri(1, 1) == 1.0f && worldOri(2, 2) == 1.0f &&
				worldOri(0, 1) == 0.0f && worldOri(0, 2) == 0.0f &&
				worldOri(1, 0) == 0.0f && worldOri(1, 2) == 0.0f &&
				worldOri(2, 0) == 0.0f && worldOri(2, 1) == 0.0f) {
				m_localPosition += trans;
			}
			else {
				m_localPosition += trans * worldOri;
			}
		}
		else {
			m_localPosition += trans;
