...
// External declarations for GPU functions
extern "C" {
	struct GPUShader *GPU_pass_shader(struct GPUPass *pass);
	struct GPUPass *GPU_material_get_pass(struct GPUMaterial *mat);
}

KX_WorldInfo::KX_WorldInfo(Scene *blenderscene, World *blenderworld)
	:m_scene(blenderscene),
	m_sunLight(nullptr),
	m_lastSunPosition(mt::zero3),
	m_sunLightFound(false),
	m_sunLightSearched(false)
{
	if (blenderworld) {
...
void KX_WorldInfo::RenderBackground(RAS_Rasterizer *rasty)
{
	if (m_hasworld) {
		if (m_scene->world->skytype & (WO_SKYBLEND | WO_SKYPAPER | WO_SKYREAL)) {
			...
			// Update sun position uniform if we have a sun light
...
void KX_WorldInfo::FindSunLight(KX_Scene *scene)
{
	if (m_sunLightSearched) {
		return;
	}
	
	m_sunLightSearched = true;
	m_sunLightFound = false;
	m_sunLight = nullptr;
	...
	for (KX_LightObject *light : lightlist) {
		if (light->GetName() == "[Sun]") {
			m_sunLight = light;
			m_sunLightFound = true;
			m_lastSunPosition = light->NodeGetWorldPosition();
			printf("KX_WorldInfo: Found [Sun] light at position (%.2f, %.2f, %.2f)\n",
			       m_lastSunPosition.x, m_lastSunPosition.y, m_lastSunPosition.z);
			break;
		}
	}
	
	if (!m_sunLightFound) {
		printf("KX_WorldInfo: No [Sun] light found in scene\n");
	}
}

void KX_WorldInfo::UpdateSunUniform(GPUMaterial *gpumat)
{
	if (!gpumat) {
		return;
	}
	
	// Get the pass from the material using external function
	GPUPass *pass = GPU_material_get_pass(gpumat);
	if (!pass) {
		return;
	}
	
	// Get the shader from the pass
	GPUShader *shader = GPU_pass_shader(pass);
	if (!shader) {
		return;
	}
	
	// If we have a sun light, update the uniform
	if (m_sunLightFound && m_sunLight) {
		RAS_ILightObject *lightData = m_sunLight->GetLightData();
		mt::vec3 currentPos;
		
		if (lightData && lightData->m_type == RAS_ILightObject::LightType::LIGHT_SUN) {
			// For SUN type lights, the direction is the -Z axis of the object (forward in Blender)
			// We multiply by a fixed value to represent a large distance in the sky
			currentPos = -(m_sunLight->NodeGetWorldOrientation().GetColumn(2)) * 1000.0f;
		}
		else {
			currentPos = m_sunLight->NodeGetWorldPosition();
		}
		
		// Only update if position/direction changed
		if (currentPos != m_lastSunPosition) {
			m_lastSunPosition = currentPos;
		}
		
		// Update the uniform
		int loc = GPU_shader_get_uniform(shader, "u_SunPosition");
		if (loc != -1) {
			GPU_shader_uniform_vector(shader, loc, 3, 1, m_lastSunPosition.Data());
		}
	}
	else {
		// Set a default sun position (overhead)
		static mt::vec3 defaultSunPos(0.0f, 0.0f, 100.0f);
		int loc = GPU_shader_get_uniform(shader, "u_SunPosition");
		if (loc != -1) {
			GPU_shader_uniform_vector(shader, loc, 3, 1, defaultSunPos.Data());
		}
	}
}
