...
KX_FontObject::KX_FontObject(void *sgReplicationInfo,
	...
	m_textUserDirty(true),
	m_textVersion(0),
	m_lastLayer(-1),
	m_lastColor(mt::vec4(-1.0f, -1.0f, -1.0f, -1.0f)),
	m_lastFontId(-1),
	m_lastDpi(-1),
	m_lastFSize(-1.0f),
	m_lastResolution(-1.0f),
	m_lastLineSpacing(-1.0f),
	m_lastWorldScale(mt::vec3(0.0f, 0.0f, 0.0f)),
	m_lastWorldOrientation(mt::mat3::Identity()),
	m_lastWorldPosition(mt::vec3(0.0f, 0.0f, 0.0f)),
	m_lastTextVersion(-1),
	m_object(ob),
	m_dpi(72),
	m_resolution(1.0f),
...
void KX_FontObject::UpdateBuckets()
{
	...
	const bool renderDirty = m_sgNode->IsDirty(SG_Node::DIRTY_RENDER);
	...
	const mt::vec4 colorVec(color);

	const bool layerChanged = (m_layer != m_lastLayer);
	const bool colorChanged = (colorVec != m_lastColor);
	const bool textChanged = (m_textVersion != m_lastTextVersion);

	const bool fontParamsChanged = renderDirty ||
		(m_fontid != m_lastFontId) ||
		(m_dpi != m_lastDpi) ||
		(m_fsize != m_lastFSize) ||
		(m_resolution != m_lastResolution) ||
		(m_line_spacing != m_lastLineSpacing);
	...
	if (colorChanged) {
		textUser->SetColor(colorVec);
		m_lastColor = colorVec;
	}

	if (textChanged) {
		textUser->SetTexts(m_texts);
		m_lastTextVersion = m_textVersion;
	}

	if (fontParamsChanged || m_textUserDirty) {
		const mt::vec3 worldScale = NodeGetWorldScaling();
		const mt::mat3 worldOri = NodeGetWorldOrientation();
		const mt::vec3 worldPos = NodeGetWorldPosition();

		if (fontParamsChanged) {
			UpdateTextAabb();
		}

		if (m_fontid != m_lastFontId) {
			textUser->SetFontId(m_fontid);
			m_lastFontId = m_fontid;
		}
		if (m_dpi != m_lastDpi) {
		...
		// Text scale should come from the object transform matrix. Multiplying
		// world scale into BLF size/aspect mixes two scaling paths and makes the
		// rendered result depend on font state in unstable ways.
		const float size = fabs(m_fsize * RES);
		const float aspect = (size > 1.0e-9f) ? (m_fsize / size) : 1.0f;
		const mt::vec3 offset = worldOri * m_offset * worldScale;
		const mt::vec3 spacing = worldOri * mt::vec3(0.0f, m_fsize * m_line_spacing, 0.0f) * worldScale[1];

		textUser->SetSize((int)std::max(1.0f, size));
		...
		m_lastLineSpacing = m_line_spacing;
		m_lastWorldScale = worldScale;
		m_lastWorldOrientation = worldOri;
		m_lastWorldPosition = worldPos;
		m_textUserDirty = false;
	}
	textUser->ActivateMeshSlots();
}


void KX_FontObject::UpdateBucketsNoOnlyShadow()
{
	UpdateBuckets();
}

void KX_FontObject::UpdateShadowBuckets()
{
}
...
void KX_FontObject::UpdateTextAabb()
{
	mt::vec2 min;
	mt::vec2 max;
	GetTextAabb(min, max);

	// Keep culling in the same local space used by the text draw path.
	// The draw call applies m_offset before world transform, so the local AABB
	// must include that offset too.
	const mt::vec2 offset2d(m_offset.x, m_offset.y);
	min += offset2d;
	max += offset2d;
