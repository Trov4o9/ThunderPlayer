...
{
	if (!m_gpuTex || !m_mtex->tex || !m_mtex->tex->ima) {
		return;
	}

	// Fast path: compara com o ponteiro cacheado da última verificação.
	// Evita o pointer chase m_mtex→tex→ima→gputexture[] na maioria dos frames.
	int target = m_isCubeMap ? TEXTARGET_TEXTURE_CUBE_MAP : TEXTARGET_TEXTURE_2D;
	GPUTexture *currentGpuTex = m_mtex->tex->ima->gputexture[target];
	if (m_gpuTex == currentGpuTex) {
		// Textura não mudou — nada a fazer
		return;
	}

	// Textura mudou — reconstrói
	Tex *tex = m_mtex->tex;
	Image *ima = tex->ima;
	ImageUser& iuser = tex->iuser;
...
{
	/* No-op in bindless mode — textures remain resident until explicitly invalidated. */
}

unsigned long long BL_Texture::GetBindlessHandle() const
{
	return GPU_texture_bindless_handle(m_gpuTex);
}
