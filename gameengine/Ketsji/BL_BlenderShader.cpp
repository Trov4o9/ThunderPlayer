...
const RAS_AttributeArray::AttribList BL_BlenderShader::GetAttribs(const RAS_Mesh::LayersInfo& layersInfo) const
{
    ...
    static thread_local GPUVertexAttribs gpuAttribsCache;
    GPU_material_vertex_attributes(m_gpuMat, &gpuAttribsCache);

    std::unordered_map<std::string, int> uvMap;
    for (const RAS_Mesh::Layer& layer : layersInfo.uvLayers) {
        uvMap[layer.name] = layer.index;
    }

    std::unordered_map<std::string, int> colorMap;
    for (const RAS_Mesh::Layer& layer : layersInfo.colorLayers) {
        colorMap[layer.name] = layer.index;
    }

    for (unsigned int i = 0; i < gpuAttribsCache.totlayer; ++i) {
        const int type = gpuAttribsCache.layer[i].type;
        const unsigned short glindex = gpuAttribsCache.layer[i].glindex;

        if (type == CD_MTFACE || type == CD_MCOL) {
            const char *attribname = gpuAttribsCache.layer[i].name;

            if (!attribname[0]) {
                if (type == CD_MTFACE) {
                    attribs.push_back({glindex, RAS_AttributeArray::RAS_ATTRIB_UV, false, static_cast<unsigned short>(layersInfo.activeUv)});
                }
                else {
                    attribs.push_back({glindex, RAS_AttributeArray::RAS_ATTRIB_COLOR, false, static_cast<unsigned short>(layersInfo.activeColor)});
                }
                continue;
            }

            if (type == CD_MTFACE) {
                auto it = uvMap.find(attribname);
                if (it != uvMap.end()) {
                    attribs.push_back({glindex, RAS_AttributeArray::RAS_ATTRIB_UV, false, static_cast<unsigned short>(it->second)});
                }
            }
            else {
                auto it = colorMap.find(attribname);
                if (it != colorMap.end()) {
                    attribs.push_back({glindex, RAS_AttributeArray::RAS_ATTRIB_COLOR, false, static_cast<unsigned short>(it->second)});
                }
            }
        }
...
void BL_BlenderShader::SetCustomGLSL(const char *code)
{
    if (code)
        m_customGLSL = code;
    else
...
{
	// Set current GPUMaterial for custom shadow shaders
	// IMPORTANT: Always set it, even if this material has no custom shader
	// This ensures materials without custom shaders don't inherit the previous material's shadow shader
	rasty->SetCurrentGPUMaterial(m_gpuMat);
...
void BL_BlenderShader::UpdateLights(RAS_Rasterizer *rasty)
{
	...
	const float *newView = (const float *)rasty->GetViewMatrix().Data();

	// Compara as 16 floats da view matrix com o cache — memcmp é ~1ns
	if (!m_lightsNeedUpdate &&
	    memcmp(m_cachedViewMat, newView, sizeof(m_cachedViewMat)) == 0)
	{
		return;  // câmera não se moveu, luzes em view space são as mesmas
	}

	// GPU_material_update_lamps espera float[4][4] — copia do flat array
	float viewmat[4][4], viewinv[4][4];
	memcpy(viewmat, newView, sizeof(viewmat));
	memcpy(viewinv, (const float *)rasty->GetViewInvMatrix().Data(), sizeof(viewinv));

	GPU_material_update_lamps(m_gpuMat, viewmat, viewinv);

	memcpy(m_cachedViewMat, newView, sizeof(m_cachedViewMat));
...
{
	bool force = false;
	RAS_IMaterial *mat = dynamic_cast<RAS_IMaterial *>(m_materialUpdateServer);
	if (mat) {
		force = mat->GetForceInstancing();
	}
	return (GPU_instanced_drawing_support() && ((m_mat->shade_flag & MA_INSTANCING) || force));
}
