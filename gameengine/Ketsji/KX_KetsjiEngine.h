...
extern double g_render_fps_limit;
...
public:
    EXP_ListValue<KX_Scene>* GetScenes() const { return m_scenes; }
	enum FlagType
	{
		FLAG_NONE = 0,
...
private:
	...
	/// time scaling parameter. if > 1.0, time goes faster than real-time. If < 1.0, times goes slower than real-time.
		...
		double m_accumulatedSkippedTime;
	...
	/// Per-frame cache: camera visibility by frustum key.
	ankerl::unordered_dense::map<std::uintptr_t, std::vector<KX_GameObject *>> m_cameraVisibilityCache;
	/// Cached viewport/scissor to avoid redundant state changes.
	std::array<int, 4> m_lastViewport{{-1, -1, -1, -1}};
	std::array<int, 4> m_lastScissor{{-1, -1, -1, -1}};
...
public:
	...
	/// The interpolation alpha value used to interpolate between frames.
	double m_interpolationAlpha;
	/**
