...
public:
	...
	{
		const size_t rangeSize = r.end() - r.begin();
		if (rangeSize > 0) {
			m_activeObjects.reserve(m_activeObjects.size() + rangeSize);
		}
	...
	{
		if (!other.m_activeObjects.empty()) {
			m_activeObjects.reserve(m_activeObjects.size() + other.m_activeObjects.size());
		}
...
std::vector<KX_GameObject *> KX_CullingHandler::Process()
{
	...
	tbb::parallel_reduce(tbb::blocked_range<size_t>(0, m_objects->GetCount(), 256), task);
