...
#ifdef WITH_PYTHON
	// Dispose ALL Python components across ALL scenes FIRST, before any C++ destruction.
	// This guarantees dispose() is called while __dict__ is still intact.
	// The GC can clear __dict__ during C++ object destruction, so we must beat it here.
	if (m_ketsjiEngine) {
		EXP_ListValue<KX_Scene> *scenes = m_ketsjiEngine->CurrentScenes();
		if (scenes) {
			for (int si = 0, sn = scenes->GetCount(); si < sn; ++si) {
				KX_Scene *scene = static_cast<KX_Scene *>(scenes->GetValue(si));
				scene->GetPythonComponentManager().DisposeComponents();
			}
		}
	}
#endif  // WITH_PYTHON
