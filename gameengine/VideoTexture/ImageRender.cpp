...
{
    if (!m_render || m_rasterizer->GetDrawingMode() != RAS_Rasterizer::RAS_TEXTURED ||
        (m_camera && (m_camera->UseViewport() || m_camera == m_scene->GetActiveCamera())))
    {
        return false;
    }
    ...
    if (m_mirror) {
        const mt::mat3 &mirrorOri = m_mirror->GetNode()->GetWorldOrientation();
        const mt::vec3 &mirrorPos = m_mirror->GetNode()->GetWorldPosition();
        const mt::vec3 &mirrorScale = m_mirror->GetNode()->GetWorldScaling();

        mt::vec3 mirrorWorldPos = mirrorPos + mirrorScale * (mirrorOri * m_mirrorPos);
        mt::vec3 mirrorWorldZ = mirrorOri * m_mirrorZ;
        const mt::vec3 &observerPos = m_observer->GetNode()->GetWorldPosition();
        float observerDist = mt::dot(mirrorWorldPos, mirrorWorldZ) - mt::dot(observerPos, mirrorWorldZ);

        if (observerDist < 0.01f) {
            return false;
        }

        mt::vec3 cameraPos = observerPos + 2.0f * observerDist * mirrorWorldZ;

        mt::vec3 mirrorWorldY = mirrorOri * m_mirrorY;
        mt::vec3 mirrorWorldX = mirrorOri * m_mirrorX;
        ...
        camtrans = mt::mat3x4(cameraOri, cameraPos).Inverse();

        mt::vec3 mirrorOffset = (mirrorWorldPos - cameraPos) * cameraOri;

        float height = std::max({ fabs(m_mirrorY[0]), fabs(m_mirrorY[1]), fabs(m_mirrorY[2]) });
        height *= mirrorScale[0];
        height *= m_mirrorHalfHeight;

        float width = std::max({ fabs(m_mirrorX[0]), fabs(m_mirrorX[1]), fabs(m_mirrorX[2]) });
        width *= mirrorScale[0]; 
    ...
    if (m_mirror) {
        ...
        RAS_FramingManager::ComputeDefaultFrustum(nearfrust, farfrust, lens, sensor_x, sensor_y,
                                                   RAS_SENSORFIT_AUTO, shift_x, shift_y, 1.0f, frustum);
    ...
    m_rasterizer->SetViewMatrix(mt::mat4::FromAffineTransform(camtrans));

    m_engine->UpdateAnimations(m_scene);
    const SG_Frustum camFrustum(projmat * mt::mat4::FromAffineTransform(camtrans));
