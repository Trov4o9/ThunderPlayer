...
public:
	...
	struct Format
	{
		uint8_t uvSize;
		uint8_t colorSize;
		// Se true, cores de vértice chegam ao shader sem normalização (0-255).
		// Se false (padrão), GL normaliza GL_UNSIGNED_BYTE para 0.0-1.0.
		bool rawColors = false;
		...
		{
			return (uvSize == other.uvSize && colorSize == other.colorSize && rawColors == other.rawColors);
		}
...
protected:
	...
	struct VertexData
	{
		...
		std::array<std::vector<mt::vec2_packed>, RAS_Texture::MaxUnits> uvs;
		...
		std::array<std::vector<Color>, RAS_Texture::MaxUnits> colors;
	} m_vertexData;
...
public:
	...
	inline void ReserveVertices(const unsigned int count)
	{
		m_vertexData.positions.reserve(m_vertexData.positions.size() + count);
		m_vertexData.normals.reserve(m_vertexData.normals.size() + count);
		m_vertexData.tangents.reserve(m_vertexData.tangents.size() + count);
		for (unsigned short i = 0; i < m_format.uvSize; ++i)
			m_vertexData.uvs[i].reserve(m_vertexData.uvs[i].size() + count);
		for (unsigned short i = 0; i < m_format.colorSize; ++i)
			m_vertexData.colors[i].reserve(m_vertexData.colors[i].size() + count);
		m_vertexInfos.reserve(m_vertexInfos.size() + count);
	}
	...
	inline void ReservePrimitiveIndices(const unsigned int count)
	{
		m_primitiveIndices.reserve(m_primitiveIndices.size() + count);
	}

	inline void AddPrimitiveIndices(const unsigned int *indices, const unsigned int count)
	{
		m_primitiveIndices.insert(m_primitiveIndices.end(), indices, indices + count);
	}
	...
	inline void ReserveTriangleIndices(const unsigned int count)
	{
		m_triangleIndices.reserve(m_triangleIndices.size() + count);
	}

	inline void AddTriangleIndices(const unsigned int *indices, const unsigned int count)
	{
		m_triangleIndices.insert(m_triangleIndices.end(), indices, indices + count);
	}
	...
	inline const unsigned int *GetTriangleIndicesData() const
	{
		return m_triangleIndices.data();
	}

	inline const mt::vec3_packed *GetPositionsData() const
	{
		return m_vertexData.positions.data();
	}
	...
	virtual int GetOpenGLPrimitiveType() const;
