                               ...
                               const RAS_AttributeArray::AttribList& attribList)
{
	...
	// glColorPointer: fixed-pipeline, sempre normaliza GL_UNSIGNED_BYTE → 0.0–1.0 no gl_Color.
	glColorPointer(4, GL_UNSIGNED_BYTE, 0, (const void *)layout.colors[0]);

	glEnableVertexAttribArray(7);
	glVertexAttribPointer(7, 4, GL_UNSIGNED_BYTE, GL_FALSE, 0, (const void *)layout.colors[0]);
	...
	for (const RAS_AttributeArray::Attrib& attrib : attribList) {
		...
		// Para RAS_ATTRIB_COLOR, respeitar o flag rawColors do layout.
		const bool normalized = (type == RAS_AttributeArray::RAS_ATTRIB_COLOR)
		                        ? (!layout.rawColors && data.normalized)
		                        : data.normalized;
		...
		if (attrib.m_texco) {
			...
			glVertexAttribPointer(loc, data.size, data.type, normalized, 0, (const void *)offset);
		}
	}
