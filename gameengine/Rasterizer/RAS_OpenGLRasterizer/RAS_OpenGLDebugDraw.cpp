...
void RAS_OpenGLDebugDraw::Flush(RAS_Rasterizer *rasty, RAS_ICanvas *canvas, RAS_DebugDraw *debugDraw)
{
	...
	{
		const auto &lines = debugDraw->m_lines;
		if (!lines.empty()) {
	...
	{
		const auto &frustums = debugDraw->m_frustums;
		if (!frustums.empty()) {
			updateVbo(m_vbos[FRUSTUMS_VBO], frustums);

			GPU_bind_vertex_array(m_vaos[FRUSTUMS_LINE_VAO]);
			GPU_shader_bind(m_frustumLineShader);
			glDrawElementsInstanced(GL_LINES, 24, GL_UNSIGNED_BYTE, nullptr, frustums.size());

			GPU_bind_vertex_array(m_vaos[FRUSTUMS_SOLID_VAO]);
			GPU_shader_bind(m_frustumSolidShader);
			glDrawElementsInstanced(GL_TRIANGLES, 36, GL_UNSIGNED_BYTE, (const void *)(sizeof(GLubyte) * 24), frustums.size());
		}
	}

	{
		const auto &aabbs = debugDraw->m_aabbs;
		if (!aabbs.empty()) {
			updateVbo(m_vbos[AABB_VBO], aabbs);
			GPU_bind_vertex_array(m_vaos[AABB_VAO]);
			GPU_shader_bind(m_frustumLineShader);
			glDrawElementsInstanced(GL_LINES, 24, GL_UNSIGNED_BYTE, nullptr, aabbs.size());
		}
	}
	...
	{
		const auto &boxes2d = debugDraw->m_boxes2d;
		if (!boxes2d.empty()) {
			updateVbo(m_vbos[BOX_2D_VBO], boxes2d);
			GPU_bind_vertex_array(m_vaos[BOX_2D_VAO]);
			GPU_shader_bind(m_box2dShader);
			glDrawArraysInstancedARB(GL_TRIANGLE_FAN, 0, 4, boxes2d.size());
		}
	}
	...
	for (const auto &text2d : debugDraw->m_texts2d) {
		glColor4fv(text2d.m_color);
		BLF_position(blf_mono_font, text2d.m_pos[0], height - text2d.m_pos[1], 0.0f);
		BLF_draw(blf_mono_font, text2d.m_text.c_str(), text2d.m_text.size());
	}
