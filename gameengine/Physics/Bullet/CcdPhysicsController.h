...
public:
	...
	bool ApplyAsyncMesh(btTriangleIndexVertexArray *meshInterface, std::vector<int>&& polygonIndexArray, std::vector<UVco>&& triFaceUVcoArray, RAS_Mesh *meshobj, RAS_Deformer *deformer, btTriangleIndexVertexArray **r_oldMeshInterface = nullptr);
...
protected:
	...
	struct MeshShapeKeyHash
	{
		...
		{
			const size_t h0 = std::hash<RAS_Mesh *>()(std::get<0>(key));
			const size_t h1 = std::hash<RAS_Deformer *>()(std::get<1>(key));
			const size_t h2 = std::hash<int>()(static_cast<int>(std::get<2>(key)));
			return h0 ^ (h1 + 0x9e3779b9 + (h0 << 6) + (h0 >> 2)) ^ (h2 + 0x9e3779b9 + (h1 << 6) + (h1 >> 2));
		}
	};

	using MeshShapeMap = ankerl::unordered_dense::map<MeshShapeKey, CcdShapeConstructionInfo *, MeshShapeKeyHash>;
...
public:
	...
	virtual bool SynchronizeMotionStatesInterpolated(float time, float alpha);
	virtual void CachePreviousTransform();
