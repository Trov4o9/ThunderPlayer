...
void BL_Converter::RemoveScene(KX_Scene *scene)
{
	...
	ClearDelayedRemovedMeshes(scene);
...
bool BL_Converter::RemoveMeshNow(KX_Scene *scene, KX_Mesh *mesh)
{
	if (!scene || !mesh) {
		return false;
	}

	const auto sceneIt = m_sceneSlots.find(scene);
	if (sceneIt == m_sceneSlots.end()) {
		return false;
	}

	SCA_LogicManager *logicmgr = scene->GetLogicManager();
	UniquePtrList<KX_Mesh>& meshObjects = sceneIt->second.m_meshobjects;
	for (UniquePtrList<KX_Mesh>::iterator it = meshObjects.begin(); it != meshObjects.end(); ++it) {
		if (it->get() == mesh) {
			if (logicmgr) {
...
void BL_Converter::ClearDelayedRemovedMeshes(KX_Scene *scene, KX_Mesh *mesh)
{
	for (std::vector<std::pair<KX_Scene *, KX_Mesh *> >::iterator it = m_delayedRemovedMeshes.begin();
	     it != m_delayedRemovedMeshes.end(); ) {
		if (it->first == scene && (!mesh || it->second == mesh)) {
			it = m_delayedRemovedMeshes.erase(it);
		}
		else {
			++it;
		}
...
bool BL_Converter::RemoveMesh(KX_Scene *scene, KX_Mesh *mesh)
{
	if (!scene || !mesh) {
		return false;
	}
	...
	UniquePtrList<KX_Mesh>& meshObjects = sceneIt->second.m_meshobjects;
	const auto meshIt = std::find_if(meshObjects.begin(), meshObjects.end(),
	                                 [mesh](const std::unique_ptr<KX_Mesh>& meshObject) {
		                                 return meshObject.get() == mesh;
	                                 });
	if (meshIt == meshObjects.end()) {
		return false;
	}

	for (const std::pair<KX_Scene *, KX_Mesh *>& pending : m_delayedRemovedMeshes) {
		if (pending.first == scene && pending.second == mesh) {
			return false;
		}
	}
	...
	mesh->InvalidateProxy();
	m_delayedRemovedMeshes.emplace_back(scene, mesh);
	return true;
}

void BL_Converter::FlushRemovedMeshes()
{
	if (m_delayedRemovedMeshes.empty()) {
		return;
	}

	std::vector<std::pair<KX_Scene *, KX_Mesh *> > delayedRemovedMeshes;
	delayedRemovedMeshes.swap(m_delayedRemovedMeshes);

	for (const std::pair<KX_Scene *, KX_Mesh *>& pending : delayedRemovedMeshes) {
		RemoveMeshNow(pending.first, pending.second);
	}
}
...
{
    std::vector<KX_LibLoadStatus *> mergeQueue;
    m_threadinfo.m_mutex.Lock();
    mergeQueue.swap(m_mergequeue);
    ...
    for (KX_LibLoadStatus *libload : mergeQueue) {
        ...
        auto &converters = libload->GetSceneConverters();
        for (BL_SceneConverter &converterObj : converters) {
            allTasks.push_back({mergeScene, &converterObj});
        }
    }

    static auto task_func = [](TaskPool *pool, void *taskdata, int UNUSED(threadid)) {
        TaskData *data = (TaskData *)taskdata;
        BL_Converter *converter = (BL_Converter *)BLI_task_pool_userdata(pool);
        converter->MergeScene(data->scene, *data->converter);
    };

    for (TaskData &task : allTasks) {
        BLI_task_pool_push(m_threadinfo.m_pool, task_func, &task, false, TASK_PRIORITY_LOW);
    }
	...
	for (auto& pair : m_sceneSlots) {
		...
		for (UniquePtrList<KX_Mesh>::iterator it =  sceneSlot.m_meshobjects.begin(); it !=  sceneSlot.m_meshobjects.end(); ) {
			...
			if (mesh->Belong(libraryId)) {
				ClearDelayedRemovedMeshes(scene, mesh);
				logicmgr->UnregisterMesh(mesh);
