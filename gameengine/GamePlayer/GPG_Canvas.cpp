...
void GPG_Canvas::Init()
{
	if (m_window) {
		...
		GHOST_Rect bnds;
        m_window->getClientBounds(bnds);
        const float native = m_window->getNativePixelSize();
        g_vpw = (int)(bnds.getWidth() * native);
        g_vph = (int)(bnds.getHeight() * native);
        g_vpwi = (int)(bnds.getWidth() * native);
        g_vphi = (int)(bnds.getHeight() * native);
	}
}
...
void GPG_Canvas::SetMousePosition(int x, int y)
{
	...
	if (system && m_window) {
		...
		if (m_window->getState() == GHOST_kWindowStateFullScreen) {
			...
			if (fac > 0.0f) {
				float scaleX = (float)g_vpwi / (float)GetWidth();
				float scaleY = (float)g_vphi / (float)GetHeight();
				gx = (GHOST_TInt32)(x * scaleX / fac);
				gy = (GHOST_TInt32)(y * scaleY / fac);
			}
		}
...
void GPG_Canvas::ResizeWindow(int width, int height)
{
    if (m_window->getState() == GHOST_kWindowStateFullScreen) {
        ...
        m_area.SetRight(width - 1);
        m_area.SetTop(height - 1);
        UpdateOffScreensTwo(width, height);

        GHOST_Rect bnds;
        m_window->getClientBounds(bnds);
        const float native = m_window->getNativePixelSize();
        const int vpw = (int)(bnds.getWidth() * native);
        const int vph = (int)(bnds.getHeight() * native);
        g_vpwi = vpw;
        g_vphi = vph;
        g_vpw = width - 1;
        g_vph = height - 1;
        //m_window->setClientSize(vpw, vph);
        //printf("vpw: %d, vph: %d\n", vpw, vph);
        return;
    }
...
void GPG_Canvas::ConvertMousePosition(int x, int y, int &r_x, int &r_y, bool UNUSED(screen))
{
	...
	if (m_window->getState() == GHOST_kWindowStateFullScreen) {
		float scaleX = (float)GetWidth() / (float)g_vpwi;
		float scaleY = (float)GetHeight() / (float)g_vphi;
		rx *= scaleX;
		ry *= scaleY;
	}
