...
if(WIN32)
	option(WITH_CPYTHON_PGO "Build CPython with PGO in PCbuild/build.bat" ON)
	option(WITH_CPYTHON_LTO "Build CPython with LTCG in PCbuild/build.bat" ON)
	set(CPYTHON_MSVC_CL_FLAGS "/O2 /GL" CACHE STRING "Additional CL flags for CPython PCbuild")
	set(CPYTHON_MSVC_LINK_FLAGS "/LTCG" CACHE STRING "Additional LINK flags for CPython PCbuild")
	...
	set(PYTHON_BUILD_BAT_FLAGS -e -p ${PYTHON_ARCH} -c ${BUILD_MODE})
	if(WITH_CPYTHON_PGO)
		list(APPEND PYTHON_BUILD_BAT_FLAGS --pgo)
	endif()
	set(PYTHON_BUILD_ENV set IncludeTkinter=false)
	if(WITH_CPYTHON_LTO)
		set(PYTHON_BUILD_ENV ${PYTHON_BUILD_ENV} && set CL=${CPYTHON_MSVC_CL_FLAGS} && set LINK=${CPYTHON_MSVC_LINK_FLAGS})
	endif()
	...
	ExternalProject_Add(external_python
		...
		PATCH_COMMAND
			if not exist "${PYTHON_EXTERNALS_FOLDER_DOS}" mklink /D "${PYTHON_EXTERNALS_FOLDER_DOS}" "${DOWNLOADS_EXTERNALS_FOLDER_DOS}"
		CONFIGURE_COMMAND ""
		BUILD_COMMAND
			if exist P:\\NUL subst P: /d &&
			subst P: "${BUILD_DIR}/python/src/external_python" &&
			cd /d P:\\pcbuild &&
			${PYTHON_BUILD_ENV} &&
			call build.bat ${PYTHON_BUILD_BAT_FLAGS}
